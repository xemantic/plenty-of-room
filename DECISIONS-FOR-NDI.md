...
~~Six~~ ~~**Seven**~~ ~~**Eight**~~ **Nine** decisions this programme cannot make for itself.
...
last unmeasured dependency in the flatness verdict rather than bounding it~~ **WITHDRAWN, iteration 40**
([`C-0167`](gpd/claims/C-0167-coupled-cells-on-the-honeycomb-grillage.md),
[`C-0154`](gpd/claims/C-0154-honeycomb-grillage.md)): **that was a smeared-sheet statement.**
...
| **7** | ~~**NEW — is a 38 × 25 nm tile acceptable, in exchange for removing the last unmeasured dependency?**~~ **SUPERSEDED AS POSED, iteration 35** (`C-0144` §4, on `C-0141`) — **RE-POSED as two questions: confirm `10 × 6`, and name the scaffold** (§7) | item 12 of the queue's questions for Kazik — the only row here whose owner column names a decision rather than a task, its analysis having closed in iteration 25 (§7) | ~~Carrying the interlayer-coupling calibration as a live dependency on the flatness verdict, and the coupled result at 1 of 8 cells rather than 8 of 8~~ — **the cost of deferring has REVERSED SIGN**: the tile this question offered as its *default*, `15 × 4`, is **1.40084263×** §3's footprint, **fails** `T-5b` at the measured band's adverse end (**0.101759944**) and is **0 of 8** coupled cells at both ends of that band. **RE-READ against iteration 40 (`C-0167`, `C-0154`, `CH-0213`): the coupled-cell halves of this row are SMEARED single-layer square-lattice solves and on the honeycomb GRILLAGE ~~no coupled cell of either cross-section is flat~~ — RESTATED, iteration 43 (`C-0180`, `CH-0234`): with the raster's own 59 turn ties present **two** of `10 × 6`'s 64 cells are — a ZERO-PRESTRAIN count, and iteration 45 ([`C-0187`](gpd/claims/C-0187-the-turn-prestrain-sign-is-derived.md)) derives the turn departure's sign and leaves one **global phase**, at which `1` of `64` is flat at each phase and **`0` of `64` at both** — ~~and `15 × 4` was not re-graded coupled at all~~ **AND SINCE ITERATION 51 IT HAS BEEN** ([`C-0218`](gpd/claims/C-0218-the-tied-regrade-at-the-other-cross-section.md), `T-294`): `0 of 64` flat at the penalty link, `0 of 128` at `C-0208`'s resolved one, tightest `0.160538609` of the stroke = `0.565012343 nm`, `1.60538609×` `T-5b`; its **uncoupled** tied block is not flat either (`0.157228044 / 0.160659712`). Only the tie **count** transfers (`410` bonds against `435`, `45 / 14` against `50 / 9`), and its free stroke is exactly **`2/3`** of `10 × 6`'s, so `T-5b` is `1.5×` tighter in nm there — both columns now travel together, and the ordering is unchanged. — but the ordering STRENGTHENS, because `15 × 4`'s FREE tile is outside `T-5b` at every enhancement read on that lattice (~~0.312237799 / 0.227177955 / 0.220064299~~ **CORRECTED, iteration 52** ([`C-0219`](gpd/claims/C-0219-a-dishing-fit-and-the-parity-of-its-basis.md), [`CH-0282`](gpd/challenges/CH-0282-a-dishing-fit-assumes-an-even-raster-row-count.md)): **0.242196276 / 0.157167743 / 0.150056485**, the standing decomposition removed its rigid plane by three **independent** projections, which is the least-squares fit only where the face's three rigid modes are orthogonal, and on a corrugated honeycomb face that is a **PARITY**: it holds iff the raster-row count `m` is EVEN. Every `10 × 6` reading in this document is unaffected — `m` is even there and the decomposition is exact — and **the ordering is STRENGTHENED**, all three corrected values being smaller and all three still outside `T-5b`) where `10 × 6`'s is inside it (0.0501417315 / 0.0522223659). This row now stands on the UNCOUPLED tile alone**. **RESTATED, iteration 48 — THE PHASE-CONTINGENCY IS NOT WHAT IS LEFT, BECAUSE THE WHOLE RECOVERY IS A READING AT A NUMERICAL PENALTY** ([`C-0205`](gpd/claims/C-0205-what-link-stiffness-the-recovery-needs.md), `T-303`, [`CH-0258`](gpd/challenges/CH-0258-two-of-sixty-four-is-a-reading-at-a-penalty.md); [`C-0208`](gpd/claims/C-0208-a-bond-link-is-two-mechanisms.md), `T-310`, [`CH-0264`](gpd/challenges/CH-0264-a-per-bond-value-against-a-uniform-threshold.md)). Every count above is read at `RIGID_LINK_STIFFNESS = 1e4 pN/nm`, a **penalty** whose value the answer must not depend on; the two recovered cells cross `T-5b` only at **`834.060958`** and **`607.396049 pN/nm`**, and the largest a crossover connector's own mechanisms supply is **`254.808095 pN/nm`** — so the census over the ladder `41.4338953 / 64.7058824 / 254.808095 / 1 000 / 10 000` is **`0 / 0 / 0 / 2 / 2`**. Resolved per bond by the bond's own direction — the shear ceiling **exactly** at the `135` in-plane bonds, `629.20588`–`1365.32644 pN/nm` at the `300` through the thickness — it is **`0 of 64` at every one of five radial rungs**, the tightest cell in the corpus missing `T-5b` by **`0.198 %`** (`0.100198485`), and **the in-plane bonds forbid the recovery by themselves**: one recovered cell needs a radial constant of **`4581.61268 pN/nm`** and the other needs none that exists. **So this price is no longer *phase-contingent*; at every link a connector can supply there is no recovery to be contingent about.** **AND, iteration 50, THAT `0 of 64` IS ITSELF A READING ON TWO *TRANSFERRED* DISTRIBUTIONS AND ON THE WRONG TILE** ([`C-0212`](gpd/claims/C-0212-a-searched-distribution-at-the-resolved-link.md), `T-316`; [`C-0215`](gpd/claims/C-0215-route-b-coupled-on-its-own-stations.md), `T-322`): **searched**, the same 32 cells read **`22 of 32`** flat, tightest **`0.0647024141`** against the transferred `0.100198485`; and on **route B's own tile** a searched coupling is flat at **`27 of 48`** and **flat AND admissible at `7`** — the first non-empty conjunction this corpus has had. **No searched cell beats the UNCOUPLED tile under dropout** (`0 of 48`, `0 of 32`), which prices `C-0087`'s measured staple loss and not the coupling. **AND THE TWO SIDES OF THIS CELL ARE DELIBERATELY READ IN THE SAME STATE, iteration 45.** Both cross-sections' figures here are **untied** — `C-0180` re-grades `10 × 6` only, so giving this side its tied numbers (0.0446459684 / 0.0467367262) while `15 × 4` cannot be given any would make the row read as a measured ordering across two lattice states when only one side moved. ~~It is left in the untied state on purpose; the tied `15 × 4` re-grade that would close it is **`T-294`**, and until it runs the ordering is quoted like for like~~ **DISCHARGED, iteration 51** ([`C-0218`](gpd/claims/C-0218-the-tied-regrade-at-the-other-cross-section.md), `T-294`): `15 × 4` is now graded coupled on the **same tied lattice**, and it is **`0 of 64`** flat at the penalty link and **`0 of 128`** at `C-0208`'s resolved one, tightest **`0.160538609`** of the stroke — **`1.60538609×`** `T-5b`, against a `10 × 6` best that clears it by `0.426 %`; its **uncoupled** tied block reads **`0.157228044 / 0.160659712`**, not flat, where `10 × 6`'s reads `0.0446459684 / 0.0467367262` and is. **Only the tie COUNT transfers** — `410` staple bonds (`140` in plane, `270` through) against `435` (`135 / 300`), the 59 ties splitting `45 / 14` against `50 / 9` — and the tie's **worth** does not, the per-realisation median ratio running `0.930605361`–`0.995818215` against `C-0180`'s `0.902845544`–`0.988116016`, inside it at only `48 of 64`. **AND A CONVENTION BOTH COLUMNS MUST NOW CARRY**: `edgeX` is shared and `edgeY = m·3d/2`, so the free stroke goes as `1/m` and `15 × 4`'s is **`3.5194795 nm`** against `10 × 6`'s **`5.27921926`** — exactly `2/3` — which makes `T-5b`'s dimensionless `0.10` **`1.5×` tighter in absolute nm** at `15 × 4` (`0.35194795` against `0.527921926`). Read in nm the ordering **reverses at `10` of `128`** matched rows, **all** of them rows where **both** blocks are far outside the tolerance and **none** of them a row with either side flat — **so this decision is untouched, and both columns travel together wherever the ordering is quoted** | ~~**`10 × 6` on both flatness and published folding yield — but the footprint is a specification and only NDI holds that column**~~ **`10 × 6` on all THREE criteria this programme can rank — flatness, published folding yield and now footprint, which is 0.929467162 of §3's. There is no trade left to weigh** — **and since iteration 39 the block is a committed scadnano FILE that the reference implementation loads with zero warnings and that reads `ADMISSIBLE` at ZERO forced crossovers** (`C-0160`, `C-0164`), so a *yes* is now given against an object rather than against a pair of integers. It carries **no staple set** and `ADMISSIBLE` is a statement about this repository's rules, not about foldability — **and since iteration 40 a fourth stated omission: the file does not declare its scaffold TOPOLOGY, so it reads as LINEAR by default where the budget it was drawn to is M13's CIRCLE** ([`CH-0215`](gpd/challenges/CH-0215-the-artifact-does-not-state-its-scaffold-topology.md), open; no number and no verdict moves, `C-0168`) | *raised iteration 26; **superseded as posed and re-posed iteration 35**; **updated iteration 39**; awaiting* |
...
| **9** | **NEW — the widest tile the DEMONSTRATED turn topology can build is `36.04 nm`, not §3's `40 nm`. Is that acceptable, and if not which lever?** (§9) | `T-320` | **Nothing FAILS on it** — all three scaffold rows are flat uncoupled and flat with their tethers (`756 of 756`, `C-0207` — **re-graded at `C-0208`'s resolved per-bond link, iteration 49, and the count does not move**: `756 of 756` again at every one of three resolved rungs, `0.048606444`–`0.0960647281` of the stroke, the link worth at most `1.02460822×`, so [`CH-0265`](gpd/challenges/CH-0265-756-of-756-is-a-reading-at-the-penalty.md) is **upheld in its premise and refuted in its consequence**, [`C-0211`](gpd/claims/C-0211-the-uniform-raster-at-the-resolved-link.md), `T-315`), **and flat COUPLED at `27 of 48` searched cells, `7` of them also inside `C-0023`'s per-path allowable — the first flat-AND-admissible coupled cell this corpus has had, iteration 50** ([`C-0215`](gpd/claims/C-0215-route-b-coupled-on-its-own-stations.md), `T-322`; `0 of 48` on a transferred rule, and `0 of 48` beat the uncoupled tile, which is a statement about `C-0087`'s measured dropout and not about the coupling). What it costs is that the recommended tile's WIDTH becomes a programme choice where §3 states a number: `−21.8 %` on NDI's own M13mp18 default, `−16.7 %` on p7560, `−9.9 %` on p8064 | **None on the specification — this programme cannot rank a footprint.** What it can say is that **p8064 is already inside NDI's own stated `~7–8K` band**, so the `−9.9 %` row needs nothing new; that the two levers reaching §3's `40 nm` are a **two-length raster** (which re-opens decision 8) and the **zero-loop turn** (`0` of `186` records, `7 of 7` built cross-sections do the other thing); and that **the price line is explicitly open** | *raised iteration 48, having been carried inside §6 since iteration 47; awaiting* |
...
load path must traverse the thickness.
Re-graded on that lattice — same extent, same stations, same mandate, same normalising stroke, the **same
4 000-realisation dropout stream**, and the smeared half **reproducing the four published percentiles at
...
The **uncoupled** four-layer honeycomb block is flat at **0.0501417315** and **0.0522223659** at the two ends of the
measured 0.26–0.33 band, and the recommended cell is flat at **zero defects**, **0.0626407003**; it is the
**measured** staple dropout, acting on a tile whose across-helix rigidity is `24/7` smaller than this file believed,
that carries it past the tolerance.
**So the question this section has been answering since iteration 25 changes**: not *which* coupling, but **whether
the `10 × 6` block needs one at all** — a question this programme has never asked of a body that was already flat on
its own, and one the stroke, the lateral confinement and the output-coupling stability mandate all still want ties
...
([`C-0204`](gpd/claims/C-0204-the-anchor-azimuth-is-determined.md), `T-304`,
...
The tether's span is **not a bracket**: on the recommended `102 / 109` raster all 59 turns anchor at an
**allowed** scaffold crossover, so the span is **`0.787091706 nm`** — one value at every turn, at all eight
readings of the axial datum and at every anchor offset — and there the free tile is flat at **`16 of 16`**
surviving corners, **`0.0569497052`–`0.0689014279`** of the stroke. `0.11296458` is the `d + 2r_P` corner,
which that raster reaches at **no turn**.
`C-0204` relocated the concern to route B's own **uniform** rows (`92 / 98 / 106 bp`), whose spans reach
`3.93454333`–`4.35327572 nm`; **graded there with those tethers they read `0.0483790868`–`0.0946863482` of the
stroke — `756 of 756` cells flat**, at 21 lattice phases, three widths and twelve chain corners.
...
NDI, not a modelling one. **AND IT IS NOW ASKED AS ONE, iteration 48: it is DECISION 9 below**
([`C-0210`](gpd/claims/C-0210-fourteenth-answers-synthesis.md), `T-319`). It sat in this section, which asks
about **thickness**, and a reader looking for a **width** specification question would not have found it here;
it is not decision 8 either, which asks which of two readings of one tile's width §3 names.
...
**AND ON THE TILE THIS PROGRAMME RECOMMENDS, A COUPLED CELL IS NOW FLAT *AND* ADMISSIBLE — THE FIRST
NON-EMPTY CONJUNCTION THIS CORPUS HAS HAD, iteration 50**
([`C-0215`](gpd/claims/C-0215-route-b-coupled-on-its-own-stations.md), `T-322`).
Every coupled number above is read on the **`116 bp` block extent** of route A's drawable raster. Route B —
the only turn topology anybody has folded — has its own tile, `92 / 98 / 106 bp` wide, whose **free** tile is
flat at `756 of 756` ([`C-0211`](gpd/claims/C-0211-the-uniform-raster-at-the-resolved-link.md)). Graded
coupled on that tile, at stations **derived at each row length**, a *transferred* rule is flat at
**`0 of 48`** cells (tightest `0.100227515`) and a *searched* one at **`27 of 48`** (tightest
**`0.0687939715`**) — and **`7 of 48` are flat AND inside `C-0023`'s per-path allowable**, against `3 of 32`
on the `116 bp` block and `0 of 32` on `C-0212`'s own two flags.
**The station set could not be inherited, and one page of integer arithmetic said so before any solve**:
route B's rows carry **5** station columns against the block extent's **6**, at every one of the 21 phases,
and `T-316`'s inherited ladder phase **16** refuses a five-column placement at **2 of 3** widths — the column
count that carries **12 of the 27** flat cells
([`CH-0276`](gpd/challenges/CH-0276-a-coupled-census-inherited-a-station-set.md)). The tile substitution moves
the flat verdict at **`10 of 48`** paired cells (`0.839625798`–`1.34239093`); the three widths **rank
differently coupled than free** (`92 < 106 < 98` uncoupled, `92 < 98 < 106` coupled); and the transferred
ratio band misses **one-signed and favourably**, `8 of 48` below it and `0` above
...
coupling** — `C-0017`'s mandate is a **placement and stability** requirement and an uncoupled tile delivers
no output at all. On this tile flatness is simply **not what the coupling buys**: the searched coupling is a
**mandate the flatness survives**, at 27 cells and admissibly at 7.
...
`C-0063` searched **where** the ties attach with **how stiff** they are held fixed, and `C-0212` the reverse,
so this corpus carried a claim about their *ordering* and no measurement of their *interaction*. Searched
together the 2 × 2 reads `0.106508519` / `0.105528854` / `0.078544978` / **`0.0677344328`** of the stroke,
all out of sample: the joint corner beats **both** singles, the total is **`−0.452630313`** log units (a
factor of **`1.57244277`**), and the **interaction is `−0.1388`, `−12.96 %`, NEGATIVE** — each freedom is
worth *more* when the other is free, where the declared expectation was that they substitute. **The ordering
reverses**: `0.009241` log units for the placement against `0.3046` for the distribution, a quotient of
**`32.9583`** (`0.304553543 / 0.00924055963`), the other way round from `C-0063`'s square-lattice reading.
Taken **in sample**, where no corner carries a selection, the interaction is `−7.845 %`. And the cheap bound
decided the method with no solve at all: the determined ladder carries **55** stations, `5, 6, 5, 6, …` by
row, so at five columns the family is `6⁵ = **7 776**` — **exhaustible**, at the very cell the two tightest
readings live at.

**AND THE OTHER CROSS-SECTION IS NOW GRADED COUPLED IN THE SAME LATTICE STATE, iteration 51**
([`C-0218`](gpd/claims/C-0218-the-tied-regrade-at-the-other-cross-section.md), `T-294`) — see decision 7,
whose comparison row this discharges.
...
coupled cell is still worse than the uncoupled tile, at **64 of 64** — **SCOPE CORRECTED, iteration 51**
([`CH-0283`](gpd/challenges/CH-0283-the-uncoupled-tile-is-not-always-the-floor.md), from [`C-0218`](gpd/claims/C-0218-the-tied-regrade-at-the-other-cross-section.md) §6): `C-0109`'s own sentence says *"every coupled cell **OF THE
SAME TILE**"*, every tile the regularity has been reproduced on is one already **inside** `T-5b`, and on
the first tile that is not (`15 × 4`, uncoupled `0.157228044`) it is **`255 of 256`**. No verdict moves; and the uncoupled block is still what is
...
| coupled, under the measured staple dropout | **0 of 8 cells flat, at BOTH ends of the band**  **RESTATED, iteration 51 — GRADED COUPLED ON THE TIED HONEYCOMB GRILLAGE AT LAST** ([`C-0218`](gpd/claims/C-0218-the-tied-regrade-at-the-other-cross-section.md), `T-294`): **`0 of 64`** at the penalty link and **`0 of 128`** at `C-0208`'s resolved one, tightest **`0.160538609`** of the stroke = **`0.565012343 nm`**, `1.60538609×` `T-5b`. Its **uncoupled** tied block is **not flat either** (`0.157228044 / 0.160659712`). `410` staple bonds against `435`, ties `45 / 14` against `50 / 9`, and a free stroke exactly `2/3` of `10 × 6`'s, so `T-5b` is `1.5×` tighter here in absolute nm | ~~**4 of 8**, best **0.0680677948** — on the abstract cross-section (`C-0142`). **Re-graded on the DRAWABLE `102 / 109` raster's own 10 crossover columns (`C-0151`): 2 of 8, and the RECOMMENDED cell — one column, ten paths, equal springs — is flat at both ends of the band, `0.0773373597` at `f` = 0.30 and `0.0821458169` at `f` = 0.26**~~ **REVERSED, iteration 40 (`C-0167`, `CH-0213`): every one of those cells is a SMEARED single-layer square-lattice solve. Re-graded on the honeycomb GRILLAGE — same extent, stations, mandate, stroke and 4 000-realisation stream, the smeared half reproducing at `≤ 6.3e−10` — `0` of `64` cells clear `T-5b` against `15` of `64` smeared, the recommended cell is `0.145086839` / `0.149852804`, and the honeycomb is worse at 4 000 of 4 000 paired realisations. It is NOT a multiplier (1.06375481–2.47485493, non-monotone in the path count). **What fails is the COUPLING**: the UNCOUPLED block is flat at both ends of the band and the recommended cell is flat at ZERO defects, `0.0626407003` **— and RESTATED, iteration 43 ([`C-0180`](gpd/claims/C-0180-tied-honeycomb-coupled-regrade.md), [`CH-0234`](gpd/challenges/CH-0234-no-verdict-reverses-was-a-free-tile-statement.md)): that lattice omits the raster's own 59 covalent TURN ties, and with them present `2` of `64` cells clear `T-5b` — 0.0995744767 and 0.0998791032, both `f` = 0.30 and both rim-graded 5:1, the tighter clearing by 0.426 % on a prestrain sign nothing here fixes (`CH-0228`, `T-284`) **RESTATED, iteration 44 ([`C-0187`](gpd/claims/C-0187-the-turn-prestrain-sign-is-derived.md), `T-284`): the sign IS fixed. caDNAno's `±5 bp` is an integer approximation to a `5.25 bp` half turn, so a crossover's side of its own staple position sets the sign, and `C-0148`'s closure condition pins **58 of the 59** binaries — strictly alternating, equivalently set by the axial rim, and reproducing `C-0175` §8's *by the rim* free tile at `8.9e−10`. What is left is one **global phase**, which is a sign convention of a coordinate whose true eigenstrain is zero ([`CH-0240`](gpd/challenges/CH-0240-the-allowed-departure-is-common-mode.md)), and at it `1` of `64` is flat at each phase and **`0` of `64` at both** — so the recovery is still phase-contingent and a cell needs a zero-prestrain `p90` below `0.0979624378` to escape, against a corpus best of `0.0995744767`.**. The recommended cell is still not flat, and 62 of 64 still fail** **RESTATED, iteration 48 — THE PHASE-CONTINGENCY IS NOT WHAT IS LEFT, BECAUSE THE WHOLE RECOVERY IS A READING AT A NUMERICAL PENALTY** ([`C-0205`](gpd/claims/C-0205-what-link-stiffness-the-recovery-needs.md), `T-303`, [`CH-0258`](gpd/challenges/CH-0258-two-of-sixty-four-is-a-reading-at-a-penalty.md); [`C-0208`](gpd/claims/C-0208-a-bond-link-is-two-mechanisms.md), `T-310`, [`CH-0264`](gpd/challenges/CH-0264-a-per-bond-value-against-a-uniform-threshold.md)). Every count above is read at `RIGID_LINK_STIFFNESS = 1e4 pN/nm`, a **penalty** whose value the answer must not depend on; the two recovered cells cross `T-5b` only at **`834.060958`** and **`607.396049 pN/nm`**, and the largest a crossover connector's own mechanisms supply is **`254.808095 pN/nm`** — so the census over the ladder `41.4338953 / 64.7058824 / 254.808095 / 1 000 / 10 000` is **`0 / 0 / 0 / 2 / 2`**. Resolved per bond by the bond's own direction — the shear ceiling **exactly** at the `135` in-plane bonds, `629.20588`–`1365.32644 pN/nm` at the `300` through the thickness — it is **`0 of 64` at every one of five radial rungs**, the tightest cell in the corpus missing `T-5b` by **`0.198 %`** (`0.100198485`), and **the in-plane bonds forbid the recovery by themselves**: one recovered cell needs a radial constant of **`4581.61268 pN/nm`** and the other needs none that exists. **So this price is no longer *phase-contingent*; at every link a connector can supply there is no recovery to be contingent about.**  **AND, iteration 50, THAT COUNT IS A READING ON *TRANSFERRED* DISTRIBUTIONS AND ON THE WRONG TILE** ([`C-0212`](gpd/claims/C-0212-a-searched-distribution-at-the-resolved-link.md), `T-316`; [`C-0215`](gpd/claims/C-0215-route-b-coupled-on-its-own-stations.md), `T-322`): searched rather than transferred, the same 32 cells of this block read **`22 of 32`** flat, tightest **`0.0647024141`** against the transferred `0.100198485`; and on **route B's own tile** — the only turn topology anybody has folded — a searched coupling is flat at **`27 of 48`** and **flat AND inside `C-0023`'s per-path allowable at `7`**, the first non-empty conjunction this corpus has had. What no searched cell does is beat the **uncoupled** tile under dropout (`0 of 48`, `0 of 32`), which is a statement about `C-0087`'s measured staple loss and not about the coupling ([`CH-0272`](gpd/challenges/CH-0272-a-flat-count-is-not-an-admissible-design.md), [`CH-0273`](gpd/challenges/CH-0273-the-buildable-ratio-window-is-a-flatness-window.md)) |
...
| coupled, under the measured staple dropout | ~~1 of 8 cells flat at the 90th percentile~~ **0 of 8, at both ends of the band**  **RESTATED, iteration 51 — GRADED COUPLED ON THE TIED HONEYCOMB GRILLAGE AT LAST** ([`C-0218`](gpd/claims/C-0218-the-tied-regrade-at-the-other-cross-section.md), `T-294`): **`0 of 64`** at the penalty link and **`0 of 128`** at `C-0208`'s resolved one, tightest **`0.160538609`** of the stroke = **`0.565012343 nm`**, `1.60538609×` `T-5b`. Its **uncoupled** tied block is **not flat either** (`0.157228044 / 0.160659712`). `410` staple bonds against `435`, ties `45 / 14` against `50 / 9`, and a free stroke exactly `2/3` of `10 × 6`'s, so `T-5b` is `1.5×` tighter here in absolute nm | ~~**8 of 8**, best 0.0278431488~~ ~~**4 of 8**, best **0.0680677948**~~ **REVERSED, iteration 40 (`C-0167`, `CH-0213`): those were smeared-sheet solves; on the honeycomb grillage `0` of `64` cells clear `T-5b` against `15` of `64` smeared, and the recommended cell is `0.145086839`. The UNCOUPLED block stays flat, and the recommended cell is flat at zero defects, `0.0626407003`** **RESTATED, iteration 43 (`C-0180`, `CH-0234`): that lattice omits the raster's own 59 turn ties, and with them present the count is `2` of `64` — the recommended cell still not flat, and the recovery resting on a prestrain sign nothing here fixes (`CH-0228`, `T-284`) **RESTATED, iteration 44 ([`C-0187`](gpd/claims/C-0187-the-turn-prestrain-sign-is-derived.md), `T-284`): the sign IS fixed. caDNAno's `±5 bp` is an integer approximation to a `5.25 bp` half turn, so a crossover's side of its own staple position sets the sign, and `C-0148`'s closure condition pins **58 of the 59** binaries — strictly alternating, equivalently set by the axial rim, and reproducing `C-0175` §8's *by the rim* free tile at `8.9e−10`. What is left is one **global phase**, which is a sign convention of a coordinate whose true eigenstrain is zero ([`CH-0240`](gpd/challenges/CH-0240-the-allowed-departure-is-common-mode.md)), and at it `1` of `64` is flat at each phase and **`0` of `64` at both** — so the recovery is still phase-contingent and a cell needs a zero-prestrain `p90` below `0.0979624378` to escape, against a corpus best of `0.0995744767`.**** **RESTATED, iteration 48 — THE PHASE-CONTINGENCY IS NOT WHAT IS LEFT, BECAUSE THE WHOLE RECOVERY IS A READING AT A NUMERICAL PENALTY** ([`C-0205`](gpd/claims/C-0205-what-link-stiffness-the-recovery-needs.md), `T-303`, [`CH-0258`](gpd/challenges/CH-0258-two-of-sixty-four-is-a-reading-at-a-penalty.md); [`C-0208`](gpd/claims/C-0208-a-bond-link-is-two-mechanisms.md), `T-310`, [`CH-0264`](gpd/challenges/CH-0264-a-per-bond-value-against-a-uniform-threshold.md)). Every count above is read at `RIGID_LINK_STIFFNESS = 1e4 pN/nm`, a **penalty** whose value the answer must not depend on; the two recovered cells cross `T-5b` only at **`834.060958`** and **`607.396049 pN/nm`**, and the largest a crossover connector's own mechanisms supply is **`254.808095 pN/nm`** — so the census over the ladder `41.4338953 / 64.7058824 / 254.808095 / 1 000 / 10 000` is **`0 / 0 / 0 / 2 / 2`**. Resolved per bond by the bond's own direction — the shear ceiling **exactly** at the `135` in-plane bonds, `629.20588`–`1365.32644 pN/nm` at the `300` through the thickness — it is **`0 of 64` at every one of five radial rungs**, the tightest cell in the corpus missing `T-5b` by **`0.198 %`** (`0.100198485`), and **the in-plane bonds forbid the recovery by themselves**: one recovered cell needs a radial constant of **`4581.61268 pN/nm`** and the other needs none that exists. **So this price is no longer *phase-contingent*; at every link a connector can supply there is no recovery to be contingent about.**  **AND, iteration 50, THAT COUNT IS A READING ON *TRANSFERRED* DISTRIBUTIONS AND ON THE WRONG TILE** ([`C-0212`](gpd/claims/C-0212-a-searched-distribution-at-the-resolved-link.md), `T-316`; [`C-0215`](gpd/claims/C-0215-route-b-coupled-on-its-own-stations.md), `T-322`): searched rather than transferred, the same 32 cells of this block read **`22 of 32`** flat, tightest **`0.0647024141`** against the transferred `0.100198485`; and on **route B's own tile** — the only turn topology anybody has folded — a searched coupling is flat at **`27 of 48`** and **flat AND inside `C-0023`'s per-path allowable at `7`**, the first non-empty conjunction this corpus has had. What no searched cell does is beat the **uncoupled** tile under dropout (`0 of 48`, `0 of 32`), which is a statement about `C-0087`'s measured staple loss and not about the coupling ([`CH-0272`](gpd/challenges/CH-0272-a-flat-count-is-not-an-admissible-design.md), [`CH-0273`](gpd/challenges/CH-0273-the-buildable-ratio-window-is-a-flatness-window.md)) |
| attachment stations its top face supplies | 90 | 60 |
...
**CORRECTED, iteration 40** (`C-0167`, `C-0154`). `10 × 6` being flatter is still a convenience, and the
second sentence is **wrong on the honeycomb lattice**: enhancement-free, the block dishes **0.132443428** and is
not flat, so the interlayer-coupling fraction is **load-bearing on `10 × 6` too**. What `10 × 6` buys is not the
...
**RESTATED, iteration 40** ([`C-0167`](gpd/claims/C-0167-coupled-cells-on-the-honeycomb-grillage.md),
[`CH-0213`](gpd/challenges/CH-0213-the-recommended-coupled-design-does-not-survive-the-honeycomb-grillage.md)):
...
**And this section's own price line has now been withdrawn TWICE** — once in iteration 38 when the graded column
counts turned out to belong to an undrawable raster, and once here when the cells they were graded on turned out to
be the wrong model. Both withdrawals run the same way: **the question loses a cost dimension and keeps its
substance.**
...
## 9. NEW — the widest tile the DEMONSTRATED turn topology can build is `36.04 nm`, not §3's `40 nm`. Is that acceptable? (`T-320`)

**This question has been in this document since iteration 47 and it has been inside §6, which asks a different
thing.** §6 asks whether §3 admits a **two-layer** tile — a question about **thickness**. What follows is a
question about **width**, and a reader looking for one would not have found it there. It is also not decision 8:
decision 8 asks *which of two readings of one tile's width §3 names*, a **convention**; this asks whether the
width that is **buildable at all** is acceptable. Separated on `T-319`.

### The finding

§3's parameter row is *"Tile footprint — 40 × 40 nm (test tiles up to ~70 × 100 nm)"*. The tilde is on the
**test tiles**, not on the nominal, so `40 nm` is stated without a tolerance.

A honeycomb raster turn costs **unpaired scaffold**, and the only turn topology anybody has demonstrated spends
**28 nt** at every turn ([`C-0193`](gpd/claims/C-0193-the-built-turn-is-a-tether.md), `T-296`;
[`C-0200`](gpd/claims/C-0200-the-file-draws-and-the-table-orders.md), `T-302`). At that allowance the scaffold
buys `perHelix − 28` paired bases, so the row length — and therefore the tile's width — is set by the
**scaffold's length** ([`C-0201`](gpd/claims/C-0201-the-tether-is-a-load-not-a-spring.md) §7, `T-299`):

| scaffold | nt | uniform row | width | against §3's 40 nm | uncoupled, `f = 0.30` |
|---|---|---|---|---|---|
| **M13mp18** — NDI's own default | 7 249 | **92 bp** | **`31.28 nm`** | **`−21.8 %`** | `0.0425678289`, **flat** |
| p7560 | 7 560 | 98 bp | `33.32 nm` | `−16.7 %` | `0.0422200543`, flat |
| **p8064** — the top of NDI's own stated band | 8 064 | **106 bp** | **`36.04 nm`** | **`−9.9 %`** | `0.0451172785`, flat |
...
`T-315`). **AND, iteration 50, THEY ARE FLAT COUPLED TOO — AT `27 of 48` SEARCHED CELLS, `7` OF THEM ALSO INSIDE `C-0023`'s PER-PATH ALLOWABLE, THE FIRST FLAT-AND-ADMISSIBLE COUPLED CELL THIS CORPUS HAS HAD** ([`C-0215`](gpd/claims/C-0215-route-b-coupled-on-its-own-stations.md), `T-322`): `0 of 48` on a *transferred* rule (tightest `0.100227515`) against `27 of 48` searched (tightest `0.0687939715`), on stations **derived at each row length** because route B's rows carry `5` station columns against the block extent's `6`. **`0 of 48` beat the UNCOUPLED tile**, which is already flat — a statement about `C-0087`'s measured dropout and not about the coupling, and not an argument for removing it: `C-0017`'s mandate is a **placement and stability** requirement, so on this tile the coupling is a **mandate the flatness survives**. **The tile is not too floppy; it is too small.**

### Why decision 5 does not already answer it

NDI's answer to decision 5 was *"Scaffold default is M13, circular ~7-8K nucleotides, but 50K nucleotide
scaffold is available for larger systems"*, and the remedy offered for excess scaffold was *"just make the tile
thicker"*. **Thickness is the other axis** — it is §6's, and it is settled. What that answer does do is put
**p8064 inside NDI's own stated band**, so the `−9.9 %` row is reachable without asking for anything new; the
`−21.8 %` row is what the **default** delivers.

### The three levers, and what each costs on our side

1. **Accept `36.04 nm`** — order p8064 rather than M13mp18. Costs nothing this programme can price: the tile is
   flat uncoupled and flat with its tethers.
2. **A two-length raster.** The block reaches **116 bp = `39.44 nm`** (`−1.40 %`) as a box and **109 bp =
   `37.06 nm`** (`−7.35 %`) as a row span. At the demonstrated 28 nt allowance that raster needs **8 010 nt**,
   so M13mp18 is short by **761**, p7560 by **450**, and **p8064 has 54 spare**. **This lever re-opens decision
   8**, because the two numbers it delivers are exactly decision 8's two readings.
3. **The zero-loop turn** — a raster turn carrying **no** unpaired scaffold, a direct covalent step between two
   duplex ends. It needs only **6 330 nt**, fits M13mp18 with **919** spare, and reaches the same `39.44 nm`.
   It is drawable, lattice-legal and covalently reachable (`d − 2r_P = 0.718724283 nm`, at `+1.49997857 σ` of
   the measured phosphodiester step) — **and it has never been folded**: `0` of `186` unique records over `30`
   queries report it, and `7 of 7` built cross-sections do the other thing.

### What we are asking

> **Is a `36.04 nm` tile on p8064 — or a `31.28 nm` tile on the M13mp18 default — acceptable against §3's
> `40 × 40 nm`? If not, which lever do you want pulled: a two-length raster, which re-opens decision 8, or the
> undemonstrated zero-loop turn?**

**And if any of it costs something on your side, say what.** This programme has exactly one column, physics, and
it can price none of: a scaffold order, a fold with no precedent, or a footprint specification. **We are not
asking for a yes/no.** A price, a contingency on another decision, or *"the 40 is a nominal and here is its
tolerance"* are all answers this question is asking for — the first six taught us to say so, because **all three
of those that had a cost dimension came back in a shape this document had not offered**.

**What deferring costs.** Nothing here fails on it: all three rows are flat. What it costs is that the
recommended tile's width becomes a **programme** choice where §3 states a number, and every plan margin, station
census and coupled cell in the corpus is then computed at a width somebody chose — three of which are `−21.8 %`,
`−16.7 %` and `−9.9 %` from the specification and from each other.
...
(`C-0020`'s single undetermined input).~~
**UPDATED, iteration 40** ([`C-0169`](gpd/claims/C-0169-crossover-vertical-compliance.md)): **the vertical half is
delivered — as a CRITERION rather than as a measured `k_z`, and the claim says so plainly.**
The existing oxDNA trajectory cannot answer it on five checks, the decisive one being that **no estimator in this
repository computes the coordinate at all**, so the 649 MB of pruned frames would not have contained the answer
either and `T-9`'s *"at no extra cost"* was void the moment the hinge constant was filed. A measurement is priced
at **about one day on 8 cores** plus a six-line estimator, with the frames retained.
What ran instead is three orders of magnitude cheaper and settles the question the model actually asks: this
repository already **derives** a crossover's in-plane displacement stiffness from the same two phosphate bonds,
**64.7058824 pN/nm**, and the lattice's *rigid* vertical link is a penalty **154.545455×** that.
Swept alone at all 56 crossovers over four decades, the **binary reading of the constraint holds on three of four**
pre-registered predicates — it carries **0.0120392417** of the whole present-versus-absent movement and moves the
peak per-crossover force **2.93612244 %** — and **fails the fourth by 1.12787346×**: the vertical link is worth
**3.42** percentage points of the flatness convention against the **3.03** points the *entire* row-end hinge unknown
was worth, and **like it, it does not cross**.
**So `C-0009`'s rigid link is now BOUNDED rather than ASSERTED**, over the one interval of the penalty axis this
repository had never resolved
([`CH-0217`](gpd/challenges/CH-0217-the-penalty-was-never-compared-against-the-crossovers-own-displacement-stiffness.md)),
and the registration design rule it supports survives on a sharper ground than a small number: with the link
**deleted** every crossover carries **exactly zero** vertical force, so the rule rests on the link's **existence**.
**`T-9` stays live on the in-plane shear `k_s` alone**, and on a measured `k_z` if one is ever wanted. The third is
...
PREDICATE did** ([`C-0210`](gpd/claims/C-0210-fourteenth-answers-synthesis.md), `T-319`).
`ls tools/check-*.py tools/trace-answers.py` still returns **eleven**, and `tools/verify.sh` still invokes
**sixteen** distinct tools of its own without a `--self-test` guard, so both counts above are correct at this
tree. What moved is the sentence they sit in: **`tools/verify.sh` runs `./gradlew test` first** (its line 175),
and [`build.gradle.kts`](build.gradle.kts) invokes a further **twenty** distinct tools without a `--self-test`
guard — sixteen mutation harnesses, `P-31-harness-census.py` and three test fixtures — every one of which fails
that same build. The two invocation sets are **disjoint**, so **the number of distinct tools that can fail
`tools/verify.sh` is `16 + 1 + 20` = THIRTY-SEVEN**, the `+1` being `tools/test-result-reader-census.py`, which
`verify.sh` runs as a fixture rather than as a gate.
This is [`CH-0243`](gpd/challenges/CH-0243-a-checker-census-keyed-on-a-filename-prefix.md) one level further
out: that challenge found a predicate that was a **filename prefix**, and its replacement — *invocations in one
file* — is a predicate about a **file** where the question is about a **run**.
**A self-describing count can be right in its number at four consecutive re-derivations and still be answering
a question nobody asked.** The three derivations are `ls tools/check-*.py tools/trace-answers.py`; the tool
invocations in `tools/verify.sh` carrying no `--self-test` or `--selftest` flag; and the
`commandLine("$projectDir/tools/…")` invocations in `build.gradle.kts` carrying none either.

**RE-DERIVED AGAIN, iteration 52 — AND THE PREDICATE IS DATED FOR A FOURTH TIME, THIS TIME BY A WIRING IDIOM
RATHER THAN BY A FILENAME** ([`C-0220`](gpd/claims/C-0220-fifteenth-answers-synthesis.md), `T-332`;
[`CH-0286`](gpd/challenges/CH-0286-a-gate-wired-through-a-helper-is-invisible-to-the-census.md)).
The naming predicate still returns **eleven**, for the third consecutive pass. The other two moved: at the tree
this pass was written in, `tools/verify.sh` invokes **eighteen** distinct tools of its own without a
`--self-test` guard (**seventeen** gates and one fixture — `tools/cli_guard.py` is the seventeenth, wired by the
previous pass), and `build.gradle.kts` invokes **twenty-one** through a literal
`commandLine("$projectDir/tools/…")`, so the derivation in print gives `17 + 1 + 21` = **thirty-nine**.
**But the third derivation is a regular expression over a LITERAL, and since iteration 46 the Kotlin-subject
