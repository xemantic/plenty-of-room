...
1. **The tile is not a rigid plate, and the picture has to go.** It is rigid *exactly* under a uniform load —
   ...
   **one** missing path takes `C-0063`'s 0.0706 to **0.5010**, because an exhaustively optimised placement is
   a *cancellation* and a cancellation has no tolerance to a missing term. **The dropout also reverses the
   ranking**: the 34 equal springs beat the 45 two-level paths at zero defects and lose under fabrication,
