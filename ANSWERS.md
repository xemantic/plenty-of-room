...
1. **The tile is not a rigid plate, and the picture has to go.** It is rigid *exactly* under a uniform load —
   ...
   **AND THE LINE THEN RAN THREE STEPS FURTHER, ALL FAVOURABLE** (iteration 25). The tile is **design (i)
   ...
   states**~~ **AND BOTH OF ITS ARGUMENTS HAVE SINCE MOVED, iteration 45** ([`C-0154`](gpd/claims/C-0154-honeycomb-grillage.md), [`C-0167`](gpd/claims/C-0167-coupled-cells-on-the-honeycomb-grillage.md)): those two free-tile readings are smeared-sheet solves, and on the honeycomb **grillage** the free tiles are ~~**0.312237799 / 0.227177955 / 0.220064299**~~ **CORRECTED, iteration 52** ([`C-0219`](gpd/claims/C-0219-a-dishing-fit-and-the-parity-of-its-basis.md), [`CH-0282`](gpd/challenges/CH-0282-a-dishing-fit-assumes-an-even-raster-row-count.md)): **0.242196276 / 0.157167743 / 0.150056485** — the standing decomposition removed its rigid plane by three **independent** projections, which is the least-squares fit only where the face's three rigid modes are orthogonal, and on a corrugated honeycomb face that is a **PARITY**: it holds iff the raster-row count `m` is EVEN. Every `10 × 6` reading in this document is unaffected — `m` is even there and the decomposition is exact — and **the ordering is STRENGTHENED**, all three corrected values being smaller and all three still outside `T-5b` — on `15 × 4` against **0.127358454** at no enhancement and **0.0449400126 / 0.0477844467** at the calibrated ones on `10 × 6`. **The two enhancement ladders are different, so there is no matched pair to divide** — the ordering survives and is stronger (every `15 × 4` reading outside `T-5b`, every calibrated `10 × 6` reading inside it) and **no single multiple is restated here**, because a ratio assembled across two unmatched ladders would be an invention rather than a construction — and ~~**has NO threshold at all**, flat even at `f = 0` where the layers do not couple, so it does
   not depend on the calibration the `15 × 4` verdict rests on~~ **WITHDRAWN, iteration 40** ([`C-0167`](gpd/claims/C-0167-coupled-cells-on-the-honeycomb-grillage.md), [`C-0154`](gpd/claims/C-0154-honeycomb-grillage.md)) — **on the honeycomb GRILLAGE the enhancement-free block is NOT flat**; see the iteration-40 restatement below — at ~~**two-thirds of the footprint**, a
   ...
   measured folding statistics for the first time in this programme**~~ **— WITHDRAWN, iteration 40 (`C-0167`, `CH-0213`); it is the UNCOUPLED tile that is flat, and see the iteration-40 restatement below**: ~~9 of 16 graded cells at the mandated
   ...
   0 of 8 at both ends of the band, best 0.0680677948, the cross-section worth 2.13543134×.**~~ **AND RE-GRADED AGAIN, iteration 40, ON THE HONEYCOMB GRILLAGE — every coupled cell in this paragraph is a SMEARED single-layer square-lattice solve** (`C-0167`, `CH-0213`); see the iteration-40 restatement below.
   ...
   **RESTATED, iteration 35 — the four-layer numbers above are corrected in place, and this is what moved** ([`C-0141`](gpd/claims/C-0141-honeycomb-station-lattice-and-placement.md), [`C-0142`](gpd/claims/C-0142-coupled-cells-at-the-honeycomb-cross-section.md), [`CH-0174`](gpd/challenges/CH-0174-the-four-layer-cross-section-is-not-a-honeycomb.md), [`CH-0176`](gpd/challenges/CH-0176-the-first-flat-coupled-tile-is-flat-at-half-its-cells.md); was *"UNDER REVISION, iteration 33"*). **The cross-section every four-layer claim in this corpus was written on is not a honeycomb**: a honeycomb spends `3√3/4·d²` = **8.35449857 nm²** of plan per helix and the standing model spends `d²` = 6.431296, **1.29903811×**, because the in-plane row pitch is `3d/2` and the layer pitch `d√3/2` and only their **product** is the cell — so every four-layer `edgeY` was exactly **1.5×** too small. Corrected, the **footprint ordering REVERSES**: `15 × 4` is 38.08 × **56.524** nm, **1.40084263** of §3's 40.35, and `10 × 6` is 38.08 × **37.504** nm, **0.929467162** of it. The free tile dishes **0.0978155002** on `15 × 4` and **0.0240648102** on `10 × 6`; `C-0116`'s interlayer threshold moves to **0.276970522**, **inside** the measured 0.26–0.33 band, where `15 × 4` dishes **0.101759944** and **fails** `T-5b` while `10 × 6` dishes **0.0255589305** and does not. ~~The coupled evidence halves with it (`C-0142`): **four** of sixteen cells flat at the 90th percentile, **all four on `10 × 6`**, `15 × 4` **0 of 8 at both ends of the band**, best **0.0680677948**, the cross-section worth **2.13543134×**.~~ **WITHDRAWN, iteration 40 — see the iteration-40 restatement below** (`C-0167`, `CH-0213`). ~~**A coupled four-layer tile is still flat under the measured folding statistics, and now on stations a derived lattice supplies** — three of the sixteen placements are flat after snapping onto `C-0141`'s honeycomb ladder (worst snap **3.332 nm** inside a 3.57 nm ceiling), all three **equal springs**.~~ **WITHDRAWN, iteration 40** (`C-0167`, `CH-0213`) — every one of those cells is a smeared single-layer square-lattice solve; see the iteration-40 restatement below. Two further corrections travel with it: the top face carries **exactly one** rooting azimuth per helix at **30°** with **no perpendicular root anywhere**, so `C-0122`'s **90** and **60** stations are restored and `C-0128`'s oblique cost falls 6.01719478× → **2.67233333×** ([`CH-0175`](gpd/challenges/CH-0175-the-face-azimuth-is-thirty-degrees-and-there-is-one-of-it.md)); and there is **no uniform honeycomb row length at all** — an x-raster carries **both** turn senses, so 38.08 nm is a withdrawn width and design (i)'s remedy is **112 / 108 bp = 39.44 nm** axial extent, `−1.40 %` of §3's nominal ([`C-0140`](gpd/claims/C-0140-honeycomb-raster-turn-sense.md), [`CH-0172`](gpd/challenges/CH-0172-a-honeycomb-x-raster-carries-both-turn-senses.md)).
   **RESTATED AGAIN, iteration 36 — three things the corpus filed in iteration 35 and this paragraph did not carry, and all three run the FAVOURABLE way** ([`C-0146`](gpd/claims/C-0146-coupled-cells-at-the-two-length-raster.md), [`C-0147`](gpd/claims/C-0147-honeycomb-turn-slack-and-ragged-face.md)). **(1) `38.08 nm` is not a withdrawn width, it is one of two readings of the same block.** Every x-raster row of the two-length block spans **112 bp = 38.08 nm exactly** — at all ten rows and at every one of `C-0140`'s five candidate pairs — and the 116 bp = **39.44 nm** extent exceeds it by exactly a **4 bp = 1.36 nm inter-row STAGGER** that no plate model here can represent. **Which of the two §3 names is a specification question** and is now decision **8** ([`T-242`](TASKS.md)); ~~it is worth **six flat coupled cells of eight against three** through a crossover column admitted by a **0.07 nm** slack against `CrossoverLayout.EDGE_MARGIN`~~ **that price is WITHDRAWN — see the iteration-38 restatement below** ([`CH-0185`](gpd/challenges/CH-0185-a-bounding-box-crossover-column.md), answered by `C-0148`; [`CH-0195`](gpd/challenges/CH-0195-both-graded-column-counts-belong-to-an-undrawable-raster.md)), ~~and the recommended cell — one column, ten paths, equal springs — is flat at **every** reading (**0.0662801686** / **0.0708759349**, and **0.0708859619** / **0.0754995025** at the measured band's adverse end).~~ **and those four readings are smeared-sheet solves too — WITHDRAWN, iteration 40 (`C-0167`, `CH-0213`); what survives is that the width convention decides no flatness cell on either model.** **(2) The 4 bp ragged face costs §3's flatness EXACTLY ZERO**, because a four-layer block's gap-facing surface is the outermost layer's **sidewalls** while a row length moves where a helix **ends** — a coordinate in the tile plane, at right angles to the gap. The ragged faces are the tile's **RIM**, so the coefficient on the normal-direction flatness field is exactly zero and `T-5b`'s convention cannot read it; the residual channel, a 2-row rim modulation at **7.608 nm** against across-helix bending lengths of **17.2310927** and **23.2114857 nm**, is bounded at **5.54399427e−05** and **1.68371917e−05** of the stroke ~~against **0.0274976866** of headroom — a margin of **496×**~~ **— AND BOTH THE BOUND AND ITS COMPARAND HAVE MOVED, iteration 45** ([`C-0175`](gpd/claims/C-0175-drawable-raster-rim.md), `T-258`). At the recommended `102 / 109` raster the relief is `7 / 14 bp` and the bound is **`9.96901722e−05`** of the stroke — `1.79816514×`, which is **two** factors (`7/4` in the relief and `112/109` in the row span) and not one; and the **0.0274976866** headroom was that of `C-0142`'s tightest coupled cell, a state [`C-0167`](gpd/claims/C-0167-coupled-cells-on-the-honeycomb-grillage.md) has since withdrawn, so the `496×` — `275.831468×` on `C-0175`'s own arithmetic — has no live comparand. Restated against what still exists: **`262.520141×`** at the tightest cell flat with no defects and **`9177.31736×`** at the uncoupled four-layer tile. **The verdict is unchanged and it never rested on the margin** — the coefficient is exactly zero at any relief, which is a statement carrying no magnitude. What it does cost is **plan budget**: 1.36 nm against a `C-0141` outboard ceiling that saturates at **2.380 nm**, i.e. **0.571** of it at 90 demanded paths. **(3) The 28 nt turn allowance is a CHOICE**, **4.66666667×** its own reach bound of **6** — and M13 affords exactly **8**, so a uniform 112 bp row fits by two nucleotides and only strained (**6.54349121–12.112167 pN**, at or past the 10 pN unzip allowable). The yield half is **declared unpriceable** and the **threshold 8 nt** is quoted instead. **And one comparison above is cross-convention** ([`CH-0191`](gpd/challenges/CH-0191-a-width-advantage-across-two-conventions.md)): `C-0140`'s `−1.40 %` *"beats the square lattice's `−4.80 %`"* reads a **bounding box** against a **row length** — a uniform square-lattice raster has no stagger, so read on the row length the two are **both 38.08 nm and the advantage is exactly zero**, and read on the bounding box the honeycomb wins by the **1.36 nm stagger itself**. See §3 row (g) and the NDI table.
   **RESTATED AGAIN, iteration 38 — the recommended raster is `102 / 109 bp`, not 112 / 108, and the WIDTH FINDING DOES NOT MOVE** ([`C-0151`](gpd/claims/C-0151-closing-raster-selection.md), [`C-0148`](gpd/claims/C-0148-face-bond-class-residues-and-row-span-columns.md), [`C-0152`](gpd/claims/C-0152-forced-scaffold-crossover-price.md); [`CH-0194`](gpd/challenges/CH-0194-the-filter-and-closure-are-disjoint.md), [`CH-0195`](gpd/challenges/CH-0195-both-graded-column-counts-belong-to-an-undrawable-raster.md)). **`112 / 108` does not close on caDNAno's own `±5 bp` scaffold-crossover rule**: it would need **10** of the block's **59** raster crossovers **forced**. Closure depends on the two row lengths only through their residues **modulo 21**, so the family is **441** cases and needs no solve — exactly **three** residue pairs close, they are the same on both 60-helix cross-sections, and **all three differ by `14 (mod 21)`**, so **the minimum stagger a DRAWABLE two-length honeycomb raster can carry is 7 bp = 2.38 nm** where `C-0140`'s selection filter admitted **at most 4**: the filter and the closing family are **exactly disjoint** and could not have returned a buildable design at any lengths (`CH-0194`). Selected inside the closing family, **`102 / 109`** ties at the **same 116 bp = 39.44 nm** extent and `−1.40 %` — **so nothing in the width finding moves** — and is strictly best among the three pairs that tie there: **10** crossover columns against 9 and 6, **55** stations against 50 and 40, a 7 bp stagger against 14 and 28. **The whole cost of closure is ONE crossover column**, because the two pairs give the **same tile** and differ only in the **interface window**, 102 bp against 108: ~~**2** flat cells of eight against 3, a uniform **1.0567397–1.09611647×** of the dishing, and **the recommended cell survives at both ends of the measured band** — **0.0773373597** at `f = 0.30` and **0.0821458169** at `f = 0.26`, against `T-5b`'s 0.10 and uncoupled references of **0.0281953496** and **0.0299114053**.~~ **REVERSED, iteration 40** (`C-0167`, `CH-0213`): those are smeared-sheet readings; on the honeycomb grillage the count is **0 of 8** and the recommended cell is **0.145086839** / **0.149852804** — see the iteration-40 restatement below. **TIED, iteration 43** ([`C-0180`](gpd/claims/C-0180-tied-honeycomb-coupled-regrade.md)): with the raster's own **59 turn ties** present the same eight cells read **1 of 8** and the recommended cell **0.138518264** / **0.144085797**, still not flat. **The column-count comparison and the raster SELECTION are untouched**, being arithmetic on the closure lattice. The ladder phase is **determined at 16** with the **14 bp** inter-row offset, carrying **55 of 60** stations (`0.0868025325` when the recommended cell is snapped onto it, a **12 %** cost, ~~and still flat~~ — a **smeared** reading; the whole coupled family is re-graded in the iteration-40 restatement below), and closure **buys** 270 nt of scaffold — **6 330 nt** of M13's 7 249 on `10 × 6`, **919** spare — which reads as **15** unpaired nucleotides per helix against 10 at 112 / 108 and 8 at a uniform 112 bp row, so the strained uniform-row loop route of the previous paragraph's item (3) **gains** headroom. The blunt-end stacking clearance goes from **0.18** of a base-pair rise — below the design language's own quantum, so by this project's rule **not a quotable margin at all** — to **3.18**. **And closure is a reason to PREFER `102 / 109`, not a proof that `112 / 108` is unbuildable** (`C-0152`): caDNAno's rule is that software's **default**, the forcing tool exists in the same paper, and the **elastic** price of forcing is `0.350894669 k_BT` per crossover — **sub-thermal**, `2.84985806×` below one `k_BT` — with all ten costing **`0.438634952`** of one crossover column of a host sheet that folds. What is unpriced is the **kinetic** half, and `C-0152` records a negative existence result for it over **68** queries in **7** families. **Both graded column counts in the corpus, 11 and 12, were read at a raster that cannot be drawn** (`CH-0195`); at the drawable one the count is **10** at all three `EDGE_MARGIN` conventions — 0.05, 0.17 and 0.34 nm, slack **2.45 / 2.21 / 1.87 nm** — so the numerical guard decides nothing, and **decision 8's threshold goes with it**.

   **RESTATED AGAIN, iteration 39 — the recommended raster is now a FILE, its closure is CHECKED ON THE FILE, and the function that would have checked it had been passing a foreign design for the WRONG REASON** ([`C-0160`](gpd/claims/C-0160-scadnano-writer.md), [`C-0164`](gpd/claims/C-0164-lattice-aware-buildability.md), [`C-0161`](gpd/claims/C-0161-mechanics-on-an-imported-design.md)). The `10 × 6` block is emitted as `gpd/designs/gen1-block-honeycomb-10x6-102-109.sc`, and the **reference** scadnano implementation (0.21.1) loads it — and this corpus's own 15 × 112 bp sheet — with **zero warnings**, counting **49** staple crossings and **14** scaffold crossings on the sheet with its own parser: a third implementation agreeing with this corpus's census. `read → write → read` reproduces every lattice fact at **integer equality with no tolerance**, so the writer is the reader's *inverse* rather than a plausible second implementation, and the block's 55 stations, phase 16, 14 bp offset, 7 bp stagger, 102 bp interface window and **116 bp = 39.44 nm** extent are re-derived **on the emitted design**. **`C-0148`'s `±5 bp` closure is now derived from the file and the block reads `ADMISSIBLE` at ZERO forced crossovers** (`C-0164`): its **59** raster crossovers reduce by their own bond class to exactly **two** residues, `{4, 14}` — ten apart, which *is* the closure condition — and one `b₀` admits them; the withdrawn `112 / 108` fails at exactly the **10** forced crossovers `C-0148` found, *while passing the per-element rule*. The square and honeycomb width rules turn out to be **one statement**, `N ≡ step·Δ + {0, ±2·offset} (mod period)` — `C-0086`'s odd multiples of 16 bp at `offset = 0` and `C-0136`'s `7Δ + {0, 10, 11}` at `offset = 5`. **What moved is the GROUND, not the verdict**: `C-0160`'s declared falsifier `F2` **fired**, `checkBuildability()` having been **lattice-blind** — applying the square sheet's odd-multiple-of-16-bp rule to a honeycomb design — and grading the one design in this tree that nobody here drew found it passing `scadnano.origami_rectangle` on a **144 bp row width that is neither that design's 128 bp span nor any of its 48 / 80 / 128 bp scaffold runs**. **Two honesty clauses travel with this**: `ADMISSIBLE` means *"every rule this repository has for this design's lattice applies, and passes"* and **not** that anything is foldable; and the emitted block **carries no staple set**, so it is a lattice artifact rather than a design somebody could order (`C-0164`, `C-0160` §6). **A FOURTH omission is added in iteration 40** ([`CH-0215`](gpd/challenges/CH-0215-the-artifact-does-not-state-its-scaffold-topology.md), open): the emitted scaffold strand carries `domains`, `is_scaffold` and `color` but **not scadnano's `circular` property**, so the file asserts a **LINEAR** scaffold by default while the budget it was drawn to is **M13's circle** — 6 330 of 7 249 nt with 919 spare. Both readings give the same 60 domains and the same verdict (`C-0168`), so nothing in the artifact is wrong; what is missing is the **statement**, and it is the difference between two fabrication orders. 
   **RESTATED AGAIN, iteration 40 — THE COUPLED FLATNESS VERDICT IS REVERSED AND THE UNCOUPLED ONE IS NOT** ([`C-0167`](gpd/claims/C-0167-coupled-cells-on-the-honeycomb-grillage.md), [`C-0154`](gpd/claims/C-0154-honeycomb-grillage.md); [`CH-0213`](gpd/challenges/CH-0213-the-recommended-coupled-design-does-not-survive-the-honeycomb-grillage.md)). **Every coupled cell above was solved on a SMEARED single-layer square-lattice sheet.** `OrigamiSheet.acrossHelixRigidity = layers × k_θ d / p` reproduces a honeycomb block's `D_∥` at **2.8e−15** and overstates its `D_⊥` by **`24/7` = 3.42857×**, because only half a honeycomb's in-plane adjacent pairs are bonded — one layer of the block is a set of **dimers**, not a sheet, so the across-helix load path *must* traverse the thickness (`C-0154`, iteration 38). Re-graded on that lattice at the same extent, the same stations, the same mandate, the same normalising stroke and the **same 4 000-realisation dropout stream** — with the smeared half **reproducing `C-0151`'s four published percentiles at `≤ 6.3e−10` in the same process**, so the difference is the model and nothing else — the recommended cell moves **0.0773373597 → 0.145086839** at `f = 0.30` and **0.0821458169 → 0.149852804** at `f = 0.26`; **0 of 64** cells clear `T-5b`'s 0.10 at the 90th percentile against **15 of 64** smeared **— SUPERSEDED, NOT DISPUTED, iteration 43: that lattice carries the block's 435 staple crossovers and NOT the raster's own 59 covalent TURN ties, and with them present it is `2` of `64` ([`C-0180`](gpd/claims/C-0180-tied-honeycomb-coupled-regrade.md), [`CH-0234`](gpd/challenges/CH-0234-no-verdict-reverses-was-a-free-tile-statement.md); see the iteration-43 restatement below)**; `C-0151`'s published *"2 flat of 8"* is **0 of 8**; and the honeycomb reads worse at **4 000 of 4 000** paired realisations. **AND IT IS NOT A MULTIPLIER, SO NO TABLE IN THIS FILE CAN BE RESCALED**: the median of the per-realisation ratios runs **1.06375481 to 2.47485493** over the 64 cells and is **not monotone in the path count** — 1.380–2.475 at one crossover column, 1.166–1.555 at two, 1.064–1.208 at three, 1.190–1.462 at five — and at six cells the ratio of the two 90th percentiles falls **below one** (0.946828178–0.96849327) while the **paired** reading on the same draws is above it (1.06375481–1.13778808, the honeycomb worse at 58.1–64.8 % of them), a disagreement in **sign** rather than in magnitude. **WHAT FAILS IS THE COUPLING, NOT THE BLOCK, AND THAT IS THE WHOLE OF WHAT SURVIVES.** The **uncoupled** four-layer honeycomb block is flat at **0.0501417315** and **0.0522223659** at the two ends of `C-0116`'s measured 0.26–0.33 band, and the recommended cell is flat at **zero defects** (**0.0626407003**): it is `C-0087`'s **measured** staple dropout, acting on a tile whose `D_⊥` is `24/7` smaller than this file believed, that carries it past the tolerance — and `C-0109`'s *"every coupled cell is worse than the uncoupled tile"* reproduces at **160 of 160** cells graded there. **So the question this paragraph has been answering since iteration 25 is the wrong one**: not *which* coupling, but **whether the `10 × 6` block needs one at all**, which this programme has never asked of a body that was already flat on its own — and the stroke, the lateral confinement and `C-0017`'s stability mandate all want ties for reasons that are not flatness. **ONE FURTHER CLAIM OF THIS PARAGRAPH GOES WITH IT**: *"`10 × 6` has no threshold at all, flat even at `f = 0`"* is a **smeared** statement — on the grillage the enhancement-free block dishes **0.132443428** at the 116 bp extent (`C-0167`) and **0.127358454** at 112 bp (`C-0154`), neither of them flat — so the interlayer-coupling calibration is **back inside** the `10 × 6` verdict and is **cleared** by it rather than absent from it. **AND THE MECHANISM HAD BEEN IN THE CORPUS FOR TWO SYNTHESIS PASSES WITH NO CARRIER IN EITHER DELIVERABLE** — `C-0154` was filed in iteration 38 and is cited nowhere in this file or in `DECISIONS-FOR-NDI.md` before this pass — because a claim about a **MODEL** has no verdict sentence to attach itself to, and a synthesis keyed on verdicts cannot see one until a later claim converts it into a verdict. **Untouched by all of this**: `C-0151`'s selection of `102 / 109`, which is arithmetic on the closure lattice; the width finding; the station census; and every uncoupled reading above.
   **RESTATED AGAIN, iteration 43 — THE COUPLED COUNT IS `2` OF `64` AND NOT `0`, BECAUSE THE LATTICE ABOVE OMITS THE RASTER'S OWN 59 TURN TIES** ([`C-0180`](gpd/claims/C-0180-tied-honeycomb-coupled-regrade.md), [`C-0175`](gpd/claims/C-0175-drawable-raster-rim.md); [`CH-0227`](gpd/challenges/CH-0227-the-honeycomb-lattice-omits-the-rasters-own-turn-ties.md), [`CH-0234`](gpd/challenges/CH-0234-no-verdict-reverses-was-a-free-tile-statement.md), [`CH-0228`](gpd/challenges/CH-0228-every-allowed-scaffold-crossover-is-a-prestrain.md)).
   `HoneycombGrillage`'s bond list is the block's **435 staple** crossovers; a raster turn carrying zero unpaired
   nucleotides **is** a scaffold crossover, between two duplexes at their **ends**, and there are `H − 1` = **59** of
   them on a 60-helix block — covalent, and in no bond list.
   ...
   of `C-0167`'s committed values at **`4.2e−9`** and the tied free tiles reproducing `C-0175`'s three at
   **`≤ 1.5e−9`**, so this is one object measured in two states — **`2` of `64`** cells clear `T-5b`'s 0.10 at the
   90th percentile.
   Both are `f = 0.30` and both **rim-graded 5:1**: **0.106041029 → 0.0995744767** at 30 paths and
   **0.101931622 → 0.0998791032** at 50; `C-0151`'s own eight `f = 0.30` abstract-grid cells go from `0 of 8` to
   **1 of 8** *(a count over `C-0180`'s own paired column restricted to the eight cells `C-0167` reports as `0 of 8`,
   not a figure `C-0180` states)*.
   **The tightest clears the tolerance by 0.426 %** — a margin of **0.000425523** of the stroke — and it is converged
   exactly where it is thin: **0 of 6** deciding-cell convergence steps move it, at a worst departure of **`4.57e−4`**,
   a factor of **9.3**.
   **AND THAT IS THE SMALLER OF TWO UNCERTAINTIES, iteration 54** ([`C-0223`](gpd/claims/C-0223-the-resolution-of-the-flatness-census.md), `T-327`): `flatAtP90` is
   **exactly** `exceedance ≤ 0.10` — derived from `coupling/DropoutRobustPlacement.kt` and checked at **1 440 of 1 440**
   committed booleans, 0 disagreeing — so the verdict is a **binomial** statement at `n = 4 000`, and these two cells
   are **392 and 398 realisations of 4 000 against 400**: one-sided binomial `p` of **0.349** and **0.471**, i.e.
   **0.42 σ** and **0.11 σ** from the threshold. At the exact two-sided 95 % Clopper-Pearson interval **both are
   UNDETERMINED**, and so are 5 of the corpus's other 17 positive flatness verdicts, against **1 of 1 421** negative
   ones. **The convergence statement above is true on the discretisation axis, where the margin is 9.3497× and
   11.9677× the noise; on the sampling axis, which is the one the verdict is a function of, it is 22.2× and 113.5×
   worse resolved.** **What survives is the ORDERING**: on the same 4 000 realisations the tie is flatter at
   **3 854** and **3 478** of them, sign tests below the double-precision floor. So the honest statement of this row
   is *"the raster's 59 turn ties make the tile measurably flatter, and no coupled cell is demonstrably flat"* —
   the `2 of 64` is **UNDETERMINED**, not withdrawn, and 62 of 64 still fail.
   **AND IT IS STILL NOT A MULTIPLIER — IN THE DIRECTION NOBODY WOULD ASSUME.** The **free tile's** own ratio is
   **0.890395426**, i.e. `1.123×`; the **median of the per-realisation ratio** across the 64 paired cells runs
   **0.902845544 to 0.988116016**, i.e. `1.012–1.108×`, a spread of **0.0852704727** with a largest departure from the
   free tile's ratio of **0.0977**.
   So the free tile is not an order statistic of the coupled answer at all but a **ceiling the coupled cells never
   reach**, and `CH-0227`'s `1.12×` **over-states the benefit at every one of the 64 cells**.
   **The tail runs the other way**: the median ratio is below one at **64 of 64** cells while the **90th percentile of
   the same per-realisation ratio is above one at 27** of them (**1.00059803** to **1.01758487**), the worst single
   realisation reads **1.15725406**, and the ties are adverse at between **0.2 %** and **27.45 %** of the ensemble
   depending on the cell.
   ~~**THE RECOVERY RESTS ON A SIGN NOTHING IN THIS REPOSITORY FIXES, so `2` of `64` is not settled** (`CH-0228`,
   [`T-284`](TASKS.md)): every **allowed** honeycomb scaffold crossover already carries **8.57142857°** of built-in
   relative roll, a prestrain is a **load** and not a stiffness, and at that departure each cell moves by at most
   **0.00242194151** of the stroke — **5.7×** the margin that decides the tighter recovered cell — so exactly
   **1 of 64** is flat at each sign and it is a **different** cell (**0.0993228684** at the negative sign,
   **0.099361573** at the positive).~~
   **RESTATED, iteration 45 — THE SIGN IS DERIVED, AND THE TWO UNIFORM READINGS ABOVE ARE BOTH OFF-LATTICE**
   ...
   crossover takes fixes the sign, and `C-0148`'s closure condition pins **58 of the 59** binaries — strictly
   alternating, **30** turns at the high rim and **29** at the low, invariant at all eight readings of the free
   conventions, and reproducing `C-0175` §8's *by the rim* free tile at **`8.9e−10`**.
   What survives is one **global phase**, and graded on the same 64 cells it gives **`1` of `64`** flat at each
   phase, **`0` of `64` at both** and **`2` of `64` phase-contingent** — so the count is **phase-contingent** rather
   than unsettled in its sign, and a cell escapes only with a zero-prestrain 90th percentile below
   **`0.0979624378`** against a corpus best of **`0.0995744767`**, at which **0 of 64 qualify**.
   **AND ITS WHOLE LADDER IS A READING AT A PENALTY, iteration 48** ([`C-0205`](gpd/claims/C-0205-what-link-stiffness-the-recovery-needs.md), `T-303`; [`C-0208`](gpd/claims/C-0208-a-bond-link-is-two-mechanisms.md), `T-310`; [`CH-0258`](gpd/challenges/CH-0258-two-of-sixty-four-is-a-reading-at-a-penalty.md), [`CH-0264`](gpd/challenges/CH-0264-a-per-bond-value-against-a-uniform-threshold.md)): every count here is taken at `RIGID_LINK_STIFFNESS = 1e4 pN/nm`, the two recovered cells cross `T-5b` only at **`834.060958`** and **`607.396049 pN/nm`**, and a crossover connector's own mechanisms supply at most **`254.808095 pN/nm`** — so the census is **`0 / 0 / 0 / 2 / 2`** over `41.4338953 / 64.7058824 / 254.808095 / 1 000 / 10 000` and **`0 of 64` at every rung of the resolved per-bond link**, the tightest cell missing `T-5b` by **`0.198 %`**. **There is no recovery left for a phase to be contingent about.**
   **And the coordinate itself is now disputed** (~~`CH-0240`, raised~~ [`CH-0240`](gpd/challenges/CH-0240-the-allowed-departure-is-common-mode.md) is **UPHELD**, iteration 45 ([`C-0190`](gpd/claims/C-0190-the-departure-is-common-mode-and-what-replaces-it.md), `T-291`)): a level displacement rotates **both** backbones
   the same way, so the allowed departure has coefficient **exactly zero** on the relative-roll coordinate this load
   is applied through. No number of `C-0175` or `C-0180` is disputed; what would replace the term is a per-beam
   torsional eigenstrain nothing has priced, and that is `T-291`.
   **AND `CH-0240` IS NOW UPHELD, IN THE SAME ITERATION AND IN THE DIRECTION THAT STRENGTHENS IT**
   ([`C-0190`](gpd/claims/C-0190-the-departure-is-common-mode-and-what-replaces-it.md), `T-291`).
   The two backbones are **antipodal at every one of the 43 level displacements** of the period — worst departure
   from `180°` is **`0.0`** — and the tie prestrain load's projection on the demanded common-mode roll is
   **exactly `0.0` at all 59 ties**, on a load that is not itself zero. The challenge's own concession that a
   relative roll merely relieves the departure *less efficiently* is **withdrawn**: `u* = d cos δ / (2 r_P) =`
   **`1.37990892`** lies outside the reachable `cos r`, so the minimum over the whole relative channel is the
   **built** state and any relative roll makes the span **worse**.
   **The replacement is priced and it is not small.** `17.1428571°` of **twist** on each of **58** interior helices,
   **uniform** in sign where the roll assignment alternates, ceiling **`8.31368089 k_BT`** over the block —
   **`1.03924948`** of one crossover column of the host sheet, which folds. Graded on the same 64 cells it gives
   **`0 of 64`** flat at **either** sign and `0` at both, against the roll reading's `1 / 1 / 0 / 2`, and per
   realisation on the shared stream it moves a cell **`226.780027×`** what the relative roll moves it at the median.
   ...
   **What the ties do NOT change**: every **uncoupled** verdict, which moves the favourable way and stays flat — the
   ...
   cell is worse than the uncoupled tile"*, which reproduces at **64 of 64** on the tied lattice
   (**SCOPE CORRECTED, iteration 51** — [`CH-0283`](gpd/challenges/CH-0283-the-uncoupled-tile-is-not-always-the-floor.md), from [`C-0218`](gpd/claims/C-0218-the-tied-regrade-at-the-other-cross-section.md) §6: `C-0109`'s own
   sentence reads *"every coupled cell **OF THE SAME TILE**"*, and this document had dropped the clause.
   Every tile the regularity reproduces on — `C-0109`'s `0.0577199433`, `C-0142`'s `0.0240648102`,
   `C-0180`'s `0.0446459684`, `C-0212`'s `0.0448134881` — is one whose **uncoupled** reading is already
   **inside** `T-5b`; on the first tile that is not, `15 × 4` at `0.157228044`, it is **`255 of 256`**, and
   the one exception is a `0.395 %` near miss that is itself nowhere near flat. Where the uncoupled tile is
   inside the tolerance a coupling can only **spend** flatness; where it is outside, it has room to buy
   some. **No flatness verdict anywhere moves**); **48 of 64** cells
   flat at the nominal with no defects, unchanged; and this paragraph's own conclusion, because **62 of 64** coupled
   cells still fail and the recommended one-column cell is among them (**0.145086839 → 0.138518264** at `f = 0.30`
   and **0.149852804 → 0.144085797** at `f = 0.26`).
   What was **wrong** was the scope of one sentence: `CH-0227`'s *"no verdict of `C-0154` or `C-0167` reverses"* is a
   **free-tile** statement, and a one-signed favourable move is exactly what reverses a cell sitting **1.93 %** over
   the tolerance (`CH-0234`).
   **RESTATED AGAIN, iteration 46 — EVERY NUMBER OF THE LAST THREE RESTATEMENTS IS ABOUT A DESIGN
   NOBODY HAS FOLDED, AND THE ONE FOLDED INSTANCE OF THIS CROSS-SECTION IS A THIRD OBJECT**
   ([`C-0193`](gpd/claims/C-0193-the-built-turn-is-a-tether.md), `T-296`;
   [`CH-0247`](gpd/challenges/CH-0247-the-tie-set-is-a-route-not-a-lattice.md)).
   The 59 turn ties are a property of a **route**, not of the honeycomb lattice, and they exist only if the raster
   turns carry **zero** unpaired nucleotides. Read off the built `10 × 6` block's own strand diagram — Douglas et
   al.'s Supplementary Fig. S4, the monolith, whose cross-section inset **is** the `10 × 6` — the **scaffold**
   occupies bases `14 → 140` of every helix (**126**, the paper's own allotment) and the **staples** `28 → 126`
   (**98**, its paired count). So the scaffold **does** turn at the rim with no topological loopout, and the two
   **duplex** ends it joins are **28** unpaired nucleotides apart, **14 on each side**: the built covalent link sits
   **`14 bp = 4.76 nm` outboard of the duplex end, on both helices**, which is neither `C-0175`'s tie at `s = ±L/2`
   nor a flexible loop between two duplex ends. **The distinction is the STAPLES, not the scaffold.**
   **Nothing refuses the zero-loop turn**: `d − 2r_P = 0.718724283 nm` is at **`+1.49997857 σ`** of `T-71`'s
   measured C2′-endo phosphodiester step, inside its 99th percentile; the `102 / 109` raster closes on caDNAno's
   own `±5 bp` rule; and the tool draws it. What refuses it is that **nobody has folded it** — `0` of `186` unique
   records over `30` queries in `9` families reports the motif on this lattice, and **`7 of 7`** built cross-sections
   do the other thing. The loops' own stated purpose is *"to minimize undesired multimerization"*, not closure.
   **What the 28 nt BUYS is lattice freedom**, because an unpaired base has **no azimuth**, so no residue condition
   binds a turn flanked by unpaired scaffold — which is how the built block folds at **one** uniform row length
   where the drawable zero-loop raster must be **two-length**.
   **And the two routes are separated by the SCAFFOLD**: at the built allowance the `102 / 109` raster needs
   **8 010 nt**, so M13mp18 is short by **761**, p7560 by **450**, and **p8064 has 54 spare** — inverted, M13
   affords **15 nt** per turn, p7560 **20**, and p8064 **exactly the built 28**. At the built allowance the
   recommended raster is a **p8064** design, not an M13 one, and a uniform row on the built allowance is
   **31.28 nm** on M13 and **36.04 nm** on p8064 against §3's 40 nm.
   **A tether is not free either**: at M13's 15 nt each turn stores `1.00857129–1.48373364 k_BT`, so 59 of them
   store `59.5057061–87.5402845 k_BT` — **`7.15756436–10.5296662×`** `C-0190`'s whole rigid-duplex ceiling for
   the tied route.
   **So what moves is not a number but a CONDITION.** No value of `C-0175`, `C-0180` or `C-0190` is disputed; every
   ...
   **`0.198 %`**. **Both routes now agree on the count**, route B at `0 of 16` over four decades of link
   stiffness.
   **AND THAT `0 of 64` IS A READING ON TWO *TRANSFERRED* DISTRIBUTIONS — SEARCHED, IT IS `22 of 32`,
   iteration 49** ([`C-0212`](gpd/claims/C-0212-a-searched-distribution-at-the-resolved-link.md), `T-316`).
   The 64 readings are **32 cells × 2 distributions**, `C-0058`'s equal springs and its rim-graded 5:1,
   both *rules transferred onto* the lattice and neither an *optimum of* it. Composing `C-0135`'s smoothed
   minimax with a multi-start descent on the **true** training percentile, graded **out of sample** on
   `C-0208`'s own 4 000-realisation stream — which reproduces all 64 published readings at **`4.0E-9`** —
   the same 32 cells read **`22 of 32` flat**, tightest **`0.0647024141`** against the transferred
   `0.100198485`, i.e. **`1.45251772×`** at that very cell. **`C-0208` is not withdrawn**: it is exact on
   the two rules it grades.
   **Three things travel with that count and none is in the study's own verdict block.**
   **(1)** No cell of the sweep is flat and inside **both** per-path thresholds the study reads
   (`12 of 32` on the ratio, `3 of 32` on the peak, **`0 of 32`** on both) — **but one of those two is not
   the threshold it is named after**: `C-0060`'s `3.5 ≤ R ≤ 20` is its own **flat** ratio window, measured
   on `C-0058`'s square-lattice 45-station design, and `C-0060` places no ceiling on `R` at all
   ...
   **(2)** **`0 of 32`** searched cells beat the **uncoupled** tile at the 90th percentile
   (`0.0448134881` / `0.0469005226` against a best searched `0.0647024141`) while **`16 of 32`** beat it at
   **zero defects** (best `0.0298112409`), so the cost is `C-0087`'s measured dropout and not the coupling
   — and this is **not** an argument for removing it, `C-0017`'s mandate being a placement and stability
   requirement ([`CH-0272`](gpd/challenges/CH-0272-a-flat-count-is-not-an-admissible-design.md)).
   **(3)** The verdict is a property of the question and not of one rung: flat at **all five** of
   `C-0208`'s radial rungs, `0.0606641028`–`0.0705668742`.
   **What is NOT established is buildability at any cell**, and the real obstruction is `C-0060`'s own
   *"what fails is the ARRAY"*: **placing** a distribution spanning `191.010656` in stiffness is unpriced
   here.
   **AND ON THE TILE THIS PROGRAMME ACTUALLY RECOMMENDS A COUPLED CELL IS NOW FLAT *AND* ADMISSIBLE —
   THE FIRST NON-EMPTY CONJUNCTION THIS CORPUS HAS HAD, iteration 50**
   ([`C-0215`](gpd/claims/C-0215-route-b-coupled-on-its-own-stations.md), `T-322`).
   Every coupled number above is read on the **`116 bp` block extent** of route A's drawable raster; route
   B's tile is `92 / 98 / 106 bp` wide and its **free** tile is flat at `756 of 756`
   ([`C-0211`](gpd/claims/C-0211-the-uniform-raster-at-the-resolved-link.md)). Graded coupled on route B's
   own tile, at stations **derived at each row length**, a *transferred* rule is flat at **`0 of 48`** cells
   (tightest `0.100227515`) and a *searched* one at **`27 of 48`** (tightest **`0.0687939715`**) — and
   **`7 of 48` are flat AND inside `C-0023`'s per-path allowable**, where the same conjunction on the
   `116 bp` block is `3 of 32` and on `C-0212`'s own two flags `0 of 32`.
   **The station set could not be inherited, and one page of integer arithmetic said so before any solve**:
   route B's rows carry **5** station columns against the block extent's **6**, at every one of the 21
   phases, and `T-316`'s inherited ladder phase **16** refuses a five-column placement at **2 of 3** widths
   — the column count that carries **12 of the 27** flat cells
   ([`CH-0276`](gpd/challenges/CH-0276-a-coupled-census-inherited-a-station-set.md)). The tile substitution
   moves the flat verdict at **`10 of 48`** paired cells (`0.839625798`–`1.34239093`), the three widths
   **rank differently coupled than free** (`92 < 106 < 98` uncoupled, `92 < 98 < 106` coupled), and the
   transferred ratio band misses **one-signed and favourably**, `8 of 48` below it and `0` above
   ([`CH-0277`](gpd/challenges/CH-0277-a-ratio-transferred-between-two-lattices.md)).
   **And the count nobody should quote without the next one: `0 of 48` beat the UNCOUPLED route-B tile**
   (`0.0521565503`–`0.0576976711`), which is already flat. **That is NOT an argument for removing the
   coupling**: `C-0017`'s mandate is a **placement and stability** requirement and an uncoupled tile
   delivers no output at all, so on this tile flatness is simply **not what the coupling buys** — the
   searched coupling is a **mandate the flatness survives**, at 27 cells and admissibly at 7
   ([`CH-0272`](gpd/challenges/CH-0272-a-flat-count-is-not-an-admissible-design.md), from the other side).
   **AND THE TWO FREEDOMS ARE SYNERGISTIC, WHICH REVERSES `C-0063`'s STANDING ORDERING ON THIS LATTICE,
   iteration 50** ([`C-0216`](gpd/claims/C-0216-the-placement-and-the-distribution-together.md), `T-323`).
   `C-0063` searched the **placement** with the distribution fixed and `C-0212` the **distribution** with
   the placement fixed, so the corpus carried a claim about their *ordering* and no measurement of their
   *interaction*. Searched together at `C-0208`'s resolved per-bond link the 2 × 2 reads `0.106508519` /
   `0.105528854` / `0.078544978` / **`0.0677344328`** of the stroke, all out of sample: the joint corner
   beats **both** singles, the total is **`−0.452630313`** log units (a factor of **`1.57244277`**), and
   the **interaction is `−0.1388`, `−12.96 %`, NEGATIVE** — each freedom is worth *more* when the other is
   free, where the declared expectation was that they substitute. **The ordering reverses**: the placement
   main effect is `0.009241` log units against the distribution's `0.3046`, a quotient of
   **`32.9583`** (`0.304553543 / 0.00924055963`), the other way round from `C-0063`'s square-lattice
   reading — and the **equal-spring** ranking of placements is *anti*-correlated with what a searched
   distribution wants (Spearman `−0.09354` and `−0.438018191`) while the **rim-graded** ranking is mildly
   informative (`+0.4924`, `+0.626`). Taken **in sample**, where no corner carries a selection, the
   interaction is `−7.845 %`, so the selection is separable from it by measurement rather than by caveat.
   **The cheap bound decided the method and needed no solve**: the determined ladder carries **55**
   stations, `5, 6, 5, 6, 5, 6, 5, 6, 5, 6` by row, so at five columns the five-station rows are **forced**
   and the family is `6⁵ = 7 776` — **exhaustible**, at the very cell `C-0208` and `C-0212` are tightest
   at — and it admits **no centro-symmetric member at all**, so `C-0063`'s own search strategy has no
   analogue here.
   **AND THE OTHER CROSS-SECTION IS NOW GRADED COUPLED IN THE SAME STATE, WHICH DISCHARGES A LEAVE
   `C-0186` TOOK DELIBERATELY, iteration 51** ([`C-0218`](gpd/claims/C-0218-the-tied-regrade-at-the-other-cross-section.md), `T-294`). `C-0180` re-graded `10 × 6`
   only, so decision 7's comparison could not quote both cross-sections in one lattice state. Graded on the
   tied honeycomb grillage, `15 × 4` is **`0 of 64`** flat at the penalty link and **`0 of 128`** at
   `C-0208`'s resolved one, tightest **`0.160538609`** of the stroke — **`1.60538609×`** `T-5b`, where the
   tightest `10 × 6` cell in the corpus clears it by `0.426 %`. **Only the tie COUNT transfers**: `15 × 4`
   carries **`410`** staple bonds (`140` in plane, `270` through the thickness) against `435` (`135 / 300`),
   and its 59 turn ties split **`45 / 14`** against `50 / 9`; the tie's **worth** does not transfer either,
   the per-realisation median ratio running `0.930605361`–`0.995818215` against `C-0180`'s
   `0.902845544`–`0.988116016` and inside it at only **`48 of 64`**.
   **AND THE ROW'S OWN ACCEPTANCE CLAUSE ASKED FOR A STATE THE LATTICE REFUSES, WHICH IS A CONVENTION
   BOTH COLUMNS MUST NOW CARRY**: `edgeX` is shared and `edgeY = m·3d/2`, so the free stroke goes as `1/m`
   and is **`3.5194795 nm`** against **`5.27921926`** — **exactly `2/3`**, a theorem asserted to `1e−12` —
   which makes `T-5b`'s dimensionless `0.10` **`1.5×` tighter in absolute nm** at `15 × 4`
   (`0.35194795 nm` against `0.527921926`). Read in nm the ordering **reverses at `10` of `128`** matched
   rows — all of them rows where **both** blocks are far outside the tolerance, and **`0`** of the rows
   with either side flat — so decision 7 is untouched and **both columns travel together** wherever the
   ordering is quoted.
   **AND THE FIRST BLOCK WITH AN ODD RASTER-ROW COUNT BROKE THIS PROJECT'S BEST FALSIFIER, WHICH IS WHY
   THREE PUBLISHED `15 × 4` READINGS MOVED** ([`CH-0282`](gpd/challenges/CH-0282-a-dishing-fit-assumes-an-even-raster-row-count.md), [`C-0219`](gpd/claims/C-0219-a-dishing-fit-and-the-parity-of-its-basis.md), `T-330`). A uniform
   pressure on `15 × 4` gives an **exactly uniform** field — every face beam at `p/k_f` to `1e−10` — and
   `HoneycombDeflection` reported **`0.0620506254`** of the stroke as dishing, because it removed its rigid
   plane by three **independent** projections and `⟨piston, tiltY⟩ = ∫y dA` vanishes only where the
   corrugated gap sequence is palindromic, i.e. only where `m` is **EVEN**. Every block this corpus had
   graded has `m = 10`. Repaired as a genuine least-squares fit the falsifier reads **`0.0`**; the branch is
   taken on a **closed-form integer** statement about the face's own half-bond ladder (`Σ beamY` is exactly
   `0` at even `m` and `∓(m − 1)d/4` at odd), so **15 of the 18** result files carrying a grillage dishing
   are unmoved **by construction**, `4 of 4` byte-identity controls confirm it, **3** are re-emitted and
   **0** fourth-level consumers move. **No verdict moves at either cross-section.**
   ...
   **WITHDRAWN, iteration 48, in TWO steps that disagree with each other — and the second withdraws the
   first's reading of it** ([`C-0204`](gpd/claims/C-0204-the-anchor-azimuth-is-determined.md), `T-304`,
   [`CH-0256`](gpd/challenges/CH-0256-the-azimuth-is-not-a-convention.md);
   [`C-0207`](gpd/claims/C-0207-the-uniform-raster-is-flat-with-its-tethers.md), `T-307`,
   [`CH-0262`](gpd/challenges/CH-0262-the-alarm-did-not-survive-its-relocation.md)).
   **(i) The span is not a bracket and never was.** On the recommended `102 / 109` raster every one of the 59
   raster turns anchors at an **allowed** scaffold crossover, so its two phosphates sit at `±8.57142857°` and
   `∓171.428571°` from the line of centres and the span is **`0.787091706 nm`** — one value, at all 59 turns,
   at all eight readings of the axial datum and at **every** anchor offset, reproduced out of `T-246`'s own
   result file at `4.2e−10`. `C-0201`'s 36 corners collapse to 16 and **`16 of 16` are FLAT**:
   **`0.0569497052`–`0.0689014279`** of the stroke against the bracket's `0.0569815008`–`0.166312182`, with
   the chain's tension falling to `0.175872271`–`0.479548487 pN` from `0.160569993`–`3.03288672`.
   **`0.11296458` is read at an azimuth the design does not occupy.**
   **(ii) And the RELOCATION did not survive its own grading either.** `C-0204` moved the concern to route B's
   **uniform** rasters (`92 / 98 / 106 bp`), which do not close, so their span is a *distribution* and their
   lattice phase a free variable — and at all 21 phases the worst turn spans `3.93454333`–`4.35327572 nm`,
   essentially `C-0201`'s worst corner. **Graded there WITH those tethers the three widths read
   `0.0483790868`–`0.0946863482` of the stroke: `756 of 756` cells flat**, at every phase and every corner,
   reproducing `C-0201` §7's own untied `0.0425678289 / 0.0422200543 / 0.0451172785` at `8.6e−10` and its span
   census turn for turn at all 63 `(width, phase)` rows at `4.8e−9` nm. **The span reaching the worst corner is
   a LEVEL and the dishing is a FIELD**, and three artifacts made the step between them without taking it: the
   59 contributions are **signed** and only `50` of them have a coordinate at all, so the worst corner costs at
   most `0.0502869583` of the stroke. **The lattice phase is a real design variable worth `1.82364566×`** —
   best `5 / 16 / 9`, worst `10 / 12 / 14` — and the phase `C-0204` names best on its own alignment count,
   phase **`7`**, is the dishing optimum at **none** of the three widths, costing `1.053`–`1.354×`.
   **What stands and what does not**: `C-0204`'s span census is untouched and every determined-raster number in
   it holds; what is withdrawn is the *"relocated rather than withdrawn"* reading.
   **AND `C-0207`'s OWN COUNT WAS CHALLENGED IN THE SAME ITERATION**
   ([`CH-0265`](gpd/challenges/CH-0265-756-of-756-is-a-reading-at-the-penalty.md), **UPHELD IN ITS PREMISE and
   REFUTED IN ITS CONSEQUENCE**, iteration 49): the `756 of 756` was read at `k_link = 1e4 pN/nm`,
   **`39.2452209×`** above every rung a crossover connector can supply (`10000 / 254.808095`), carrying the very
   penalty `C-0205` refuted and `C-0208` has since resolved — and route B's own 90th percentile is known to
   **rise** as the link softens.
   **Re-graded at the resolved per-bond link the count does not move**
   ([`C-0211`](gpd/claims/C-0211-the-uniform-raster-at-the-resolved-link.md), `T-315`): at `254.808095 pN/nm`
   in plane and `629.20588`–`1365.32644` through the thickness the three widths read
   **`0.048606444`–`0.0960647281`** of the stroke, **`756 of 756` flat at every one of three resolved rungs**
   and `3024 of 3024` over a four-rung ladder whose softest member puts **every** bond at the shear ceiling,
   with the penalty control reproducing `C-0207`'s own 756 cells at **`5.0e−11`**. The link is worth
   **`1.00100801`–`1.02460822×`** of the reading, a solve-free cheap bound off `T-297`'s own committed free-tile
   sweep predicted exactly that before the grading ran (crossing threshold `0.0956283129`, `0 of 756` above it),
   and the adverse direction transfers in **sign** and not in **size** because `C-0205` §4's `0 of 16` is a
   **coupled `p90`** under measured dropout where this is the **free tile**. `C-0207`'s phase recommendation
   `b₀ = 5 / 16 / 9` is the optimum at every rung. **The count now travels with its link, and the lattice does
   not carry 435 bonds at these widths — it carries `358 / 385 / 410`**
   ([`CH-0270`](gpd/challenges/CH-0270-the-uniform-raster-does-not-carry-435-bonds.md)).
   ...
   **sign** of what the turn does.
   ...
   rather than a measurement.~~ **ANSWERED AND WITHDRAWN, iteration 48 — see (i) and (ii) above: `T-304` ran,
   the azimuth is lattice arithmetic and therefore DETERMINED, and there is no straddle on either raster.**
   ...
   **`92 bp = 31.28 nm`** on M13mp18 and **`106 bp = 36.04 nm`** on p8064 against §3's `40 nm` — and all
   three are **flat uncoupled** — and, iteration 48, flat with their own tethers too, at **`756 of 756`** cells
   ([`C-0207`](gpd/claims/C-0207-the-uniform-raster-is-flat-with-its-tethers.md)). **AND, iteration 50, THEY ARE FLAT COUPLED TOO — AT `27 of 48` SEARCHED CELLS, `7` OF THEM ALSO INSIDE `C-0023`'s PER-PATH ALLOWABLE, THE FIRST FLAT-AND-ADMISSIBLE COUPLED CELL THIS CORPUS HAS HAD** ([`C-0215`](gpd/claims/C-0215-route-b-coupled-on-its-own-stations.md), `T-322`): `0 of 48` on a *transferred* rule (tightest `0.100227515`) against `27 of 48` searched (tightest `0.0687939715`), on stations **derived at each row length** because route B's rows carry `5` station columns against the block extent's `6`. **`0 of 48` beat the UNCOUPLED tile**, which is already flat — a statement about `C-0087`'s measured dropout and not about the coupling, and not an argument for removing it: `C-0017`'s mandate is a **placement and stability** requirement, so on this tile the coupling is a **mandate the flatness survives**. **The tile is not too floppy;
   it is too small** — **and since iteration 48 that is asked as DECISION 9** rather than carried inside
   `DECISIONS-FOR-NDI.md`'s section 6, which asks about the tile's *thickness*
   ([`C-0210`](gpd/claims/C-0210-fourteenth-answers-synthesis.md), `T-319`).
...
| 5b | Deflected shape of the tile | `A8.2` | **PASS**, verdict *rigid plate rejected*. The successor question, *can it be made flat*, was **closed on every coupling axis this programme can reach** (count, distribution, placement, topology, phase) — **flat as designed**, 0.0621469105 of the stroke at the buildable width, and **not shown flat as built**, the best 90th percentile under the measured staple dropout being **0.2845** for an array and **0.375506727** for the shared body on the real lattice, against `T-5b`'s 0.10. **REOPENED AND ANSWERED FROM THE OTHER SIDE, iteration 23**: that closure is a statement about a **2 nm** tile, and §3 specifies a ~10 nm one. A **four-layer** honeycomb tile dishes ~~**0.0577199433**~~ of the stroke **with no coupling at all**, and the unspent axis was the **body**, not the coupling (`C-0109`). What survives is a 1.16× statement about the coupling under dropout, and the crossing that verdict is conditional on is now located: ~~**`f` = 0.0788618807** (`C-0116`), so the measured band clears it **monotonically** — **3.29690337×** at its adverse low end~~. See row (g) of §3 ~~**AND ON THE CROSS-SECTION ITS OWN SOURCE RECOMMENDS, A COUPLED TILE IS FLAT UNDER THE MEASURED FOLDING** (`C-0118`, `C-0119`, `C-0120`): the tile is design (i) of the caDNAno paper, that paper recommends `10 × 6` instead, `10 × 6` is **6.6× flatter with no threshold at all** at two-thirds the footprint, and **9 of 16 coupled cells are flat at the 90th percentile** — all eight on `10 × 6`, best **0.0278431488**. **The cross-section is worth 3.17109774× and the distribution has no consistent sign**~~, so the design order runs tile first, coupling second. **RESTATED, iteration 35 — the cross-section those four-layer numbers were solved on is not a honeycomb** (`C-0141`, `C-0142`, `CH-0174`, `CH-0176`): corrected, the free tile dishes **0.0978155002** on `15 × 4` and **0.0240648102** on `10 × 6`, the interlayer threshold moves to **0.276970522** — **inside** the measured 0.26–0.33 band, where `15 × 4` dishes **0.101759944** and **fails** `T-5b` — and the coupled evidence is ~~**four** of sixteen cells rather than nine, **all four on `10 × 6`**, best **0.0680677948**, the cross-section worth **2.13543134×**~~ **— REVERSED, iteration 40 (`C-0167`, `C-0154`, `CH-0213`): every one of those cells is a SMEARED single-layer square-lattice solve, and on the honeycomb GRILLAGE, whose `D_⊥` is `24/7` smaller, the count is `0` of `64` against `15` of `64` smeared and `C-0151`'s recommended cell reads 0.145086839 **— and that lattice omits the raster's own 59 covalent TURN ties: with them present it is `2` of `64`, `C-0151`'s own eight cells `1` of `8`, and the recommended cell 0.138518264, still not flat, iteration 43 ([`C-0180`](gpd/claims/C-0180-tied-honeycomb-coupled-regrade.md), [`CH-0234`](gpd/challenges/CH-0234-no-verdict-reverses-was-a-free-tile-statement.md))** **— and that `2` of `64` is a ZERO-PRESTRAIN count: with the turn departure's sign DERIVED rather than swept (iteration 45, [`C-0187`](gpd/claims/C-0187-the-turn-prestrain-sign-is-derived.md), `T-284`) one **global phase** is left, and at it `1` of `64` is flat at each phase and **`0` of `64` at both** — the recovery is phase-contingent, and the coordinate the load is applied through has coefficient **exactly zero** for a level displacement ([`CH-0240`](gpd/challenges/CH-0240-the-allowed-departure-is-common-mode.md), raised, `T-291`)**. What fails is the COUPLING: the uncoupled four-layer honeycomb block is flat at 0.0501417315 / 0.0522223659 at the two ends of the measured band, and the recommended cell is flat at zero defects (0.0626407003)**. **AND THE `2` of `64` WAS A READING AT A NUMERICAL PENALTY, iteration 48** ([`C-0205`](gpd/claims/C-0205-what-link-stiffness-the-recovery-needs.md), `T-303`, [`CH-0258`](gpd/challenges/CH-0258-two-of-sixty-four-is-a-reading-at-a-penalty.md); [`C-0208`](gpd/claims/C-0208-a-bond-link-is-two-mechanisms.md), `T-310`, [`CH-0264`](gpd/challenges/CH-0264-a-per-bond-value-against-a-uniform-threshold.md)): it is read at `RIGID_LINK_STIFFNESS = 1e4 pN/nm`, the two recovered cells cross `T-5b` at **`834.060958`** and **`607.396049 pN/nm`**, and the largest a crossover connector's own mechanisms supply is **`254.808095 pN/nm`** — so the census reads **`0 / 0 / 0 / 2 / 2`** across `41.4338953 / 64.7058824 / 254.808095 / 1 000 / 10 000`. Resolved by the **bond's own direction** — the shear ceiling exactly at the `135` in-plane bonds and `629.20588`–`1365.32644` at the `300` through the thickness — the count is **`0 of 64` at every one of five radial rungs**, the tightest cell in the corpus missing `T-5b` by **`0.198 %`** (`0.100198485`), and **the in-plane bonds forbid the recovery by themselves**: one recovered cell needs a radial constant of **`4581.61268 pN/nm`** and the other needs **none that exists**. **And route B's free-tile alarm is withdrawn** ([`C-0204`](gpd/claims/C-0204-the-anchor-azimuth-is-determined.md), [`C-0207`](gpd/claims/C-0207-the-uniform-raster-is-flat-with-its-tethers.md), [`CH-0256`](gpd/challenges/CH-0256-the-azimuth-is-not-a-convention.md), [`CH-0262`](gpd/challenges/CH-0262-the-alarm-did-not-survive-its-relocation.md)): the span is **determined** at `0.787091706 nm` and the uniform rasters are flat at **`756 of 756`** — challenged in turn at [`CH-0265`](gpd/challenges/CH-0265-756-of-756-is-a-reading-at-the-penalty.md) as a reading at the same penalty. The footprint ordering reverses with it: `10 × 6` is **0.929467162** of §3's 40.35 nm and `15 × 4` is **1.40084263×** it. | `C-0006`, `C-0009`; `C-0087`, `C-0089`, `C-0093`, `C-0098`, `C-0090`, `C-0104`, `C-0107`, `C-0109`, `C-0112`, `C-0116`, `C-0118`, `C-0119`, `C-0120`, `C-0141`, `C-0142`, `C-0154`, `C-0167` |
...
| (g) | Does the tile stay flat? | **YES, ON THE `10 × 6` FOUR-LAYER TILE — and after iteration 33–34 the cross-section is part of the answer rather than a preference.** The short answer, current as of iteration 35. **(1) The tile is four honeycomb layers, not one**: §3's own parameter row says *"Tile thickness ~10 nm (single-layer honeycomb)"* and every structural claim before iteration 23 modelled a **2 nm** sheet; one circular M13 pays for exactly four layers (`C-0109`). **(2) The cross-section every four-layer number was solved on was NOT a honeycomb, and correcting it REVERSES the footprint ordering** (`C-0141`, `CH-0174`): the in-plane row pitch is `3d/2` and the layer pitch `d√3/2`, and only their product is the cell, so every `edgeY` was exactly **1.5×** too small — `15 × 4` is **1.40084263** of §3's 40.35 nm and `10 × 6` is **0.929467162** of it. **(3) Uncoupled both are flat, and only one STAYS flat**: `15 × 4` dishes **0.0978155002** and `10 × 6` **0.0240648102** under `C-0022`'s solved collar ([`CH-0216`](gpd/challenges/CH-0216-the-recommended-block-is-graded-at-neither-of-its-widths.md), raised iteration 40: that number is graded at `edgeX` = **112 bp = 38.08 nm**, which is **neither** of the block's two width readings — its rows span 109 bp and its box 116 bp — and re-graded it is **0.0231880196** at 109 bp and **0.0231299291** at 116, **≤ 3.9 %** and *both better than the value in print*, so no verdict moves), but `C-0116`'s interlayer-coupling threshold moves to **0.276970522** — **inside** the measured 0.26–0.33 band — so at the band's adverse end `15 × 4` dishes **0.101759944** and **fails** `T-5b` while `10 × 6` dishes **0.0255589305** and does not. ~~**(4) A COUPLED tile is still flat under the measured folding statistics, on four of sixteen graded cells and all four on `10 × 6`** (`C-0142`, `CH-0176`): best **0.0680677948** at the sparsest coupling tested, `15 × 4` **0 of 8 at both ends of the band**, the cross-section worth **2.13543134×**.~~ **(4) WITHDRAWN AND REVERSED, iteration 40 — A COUPLED TILE IS NOT FLAT, AND AN UNCOUPLED ONE IS** ([`C-0167`](gpd/claims/C-0167-coupled-cells-on-the-honeycomb-grillage.md), [`C-0154`](gpd/claims/C-0154-honeycomb-grillage.md), [`CH-0213`](gpd/challenges/CH-0213-the-recommended-coupled-design-does-not-survive-the-honeycomb-grillage.md)). Every coupled cell in this row was an `OrigamiGrillage` over a **smeared equivalent sheet**, whose `acrossHelixRigidity` overstates a honeycomb block's `D_⊥` by **`24/7` = 3.42857×** — one layer of such a block is a set of **dimers**, not a sheet. Re-graded on `C-0154`'s honeycomb grillage, controlled at the same extent, stations, mandate, normalising stroke and 4 000-realisation dropout stream, with the smeared half reproducing `C-0151`'s four published percentiles at **`≤ 6.3e−10`**: **0 of 64** cells clear `T-5b` against **15 of 64** smeared, `C-0151`'s **2 of 8** is **0 of 8**, its recommended cell reads **0.145086839** / **0.149852804**, and the honeycomb is worse at **4 000 of 4 000** paired realisations. **SUPERSEDED, NOT DISPUTED, iteration 43** ([`C-0180`](gpd/claims/C-0180-tied-honeycomb-coupled-regrade.md), [`CH-0227`](gpd/challenges/CH-0227-the-honeycomb-lattice-omits-the-rasters-own-turn-ties.md), [`CH-0234`](gpd/challenges/CH-0234-no-verdict-reverses-was-a-free-tile-statement.md)): **that lattice carries the block's 435 staple crossovers and NOT the raster's own 59 covalent TURN ties**, and with the ties present the count is **`2` of `64`** — `0.106041029 → 0.0995744767` at 30 paths and `0.101931622 → 0.0998791032` at 50, both `f = 0.30` and both rim-graded 5:1 — `C-0151`'s eight cells **`1` of `8`**, and the recommended cell **0.138518264** / **0.144085797**, still not flat. The tightest clears by **0.426 %** and is converged there (0 of 6 steps, worst departure `4.57e−4`), and it is **still not a multiplier in the direction nobody would assume**: the per-realisation median ratio runs **0.902845544–0.988116016** against the free tile's **0.890395426**, so the free tile **over-states** the coupled benefit at every one of the 64 cells, while the **90th percentile** of that same ratio is **above one at 27 of 64** and one realisation reads **1.15725406**. ~~**And it is not settled**: at the **8.57142857°** every allowed honeycomb scaffold crossover carries (`CH-0228`), each cell moves by up to **0.00242194151** of the stroke — **5.7×** the deciding margin — and exactly **1 of 64** is flat at each sign, a *different* one, on a sign nothing here fixes ([`T-284`](TASKS.md)).~~ **RESTATED, iteration 45 ([`C-0187`](gpd/claims/C-0187-the-turn-prestrain-sign-is-derived.md), `T-284`; [`CH-0240`](gpd/challenges/CH-0240-the-allowed-departure-is-common-mode.md)): the sign is DERIVED — caDNAno's `±5 bp` rule and `C-0148`'s closure condition pin **58 of the 59** binaries, strictly alternating, **30** turns at the high rim and **29** at the low — so the two uniform readings above are both off-lattice and what is left is one **global phase**: **`1` of `64`** flat at each phase, **`0` of `64` at both**, **`2` of `64` phase-contingent**, and a cell escapes only below a zero-prestrain 90th percentile of **`0.0979624378`** against a corpus best of **`0.0995744767`** (**0 of 64 qualify**). **And the coordinate is disputed**: a level displacement rotates **both** backbones the same way, so the departure's coefficient on the relative-roll coordinate is **exactly zero** (~~`CH-0240`, raised~~ [`CH-0240`](gpd/challenges/CH-0240-the-allowed-departure-is-common-mode.md) is **UPHELD**, iteration 45 ([`C-0190`](gpd/claims/C-0190-the-departure-is-common-mode-and-what-replaces-it.md), `T-291`)); ~~the replacement term is a per-beam torsional eigenstrain nothing has priced, `T-291`.~~ **PRICED, iteration 45 ([`C-0190`](gpd/claims/C-0190-the-departure-is-common-mode-and-what-replaces-it.md), `T-291`)**: the replacement is `17.1428571°` of **twist** on each of 58 interior helices, and graded on these same 64 cells it gives **`0` of `64`** flat at either sign — so the two cells the tied lattice recovered do **not** survive the corrected coordinate. **It is not a multiplier** (per-realisation median ratio **1.06375481–2.47485493**, non-monotone in the path count), so no cell here can be rescaled. **What fails is the COUPLING**: the **uncoupled** four-layer honeycomb block is flat at **0.0501417315** and **0.0522223659** at the two ends of the measured band, the recommended cell is flat at **zero defects** (**0.0626407003**), and it is `C-0087`'s measured staple dropout that carries it past the tolerance. **CONDITIONED, iteration 46** ([`C-0193`](gpd/claims/C-0193-the-built-turn-is-a-tether.md), `T-296`; [`CH-0247`](gpd/challenges/CH-0247-the-tie-set-is-a-route-not-a-lattice.md)): **the 59 turn ties are a property of a ROUTE, not of the lattice**, and they exist only if the raster turns carry **zero** unpaired nucleotides. The only folded instance of this cross-section does not: its scaffold occupies `14 → 140` and its staples `28 → 126` of every helix, so the built covalent link sits **`14 bp = 4.76 nm` outboard of the duplex end** — neither a tie at `s = ±L/2` nor a loop between two duplex ends, and the distinction is the **staples**. Nothing covalent, lattice-legal or tool-side refuses the zero-loop turn (`d − 2r_P = 0.718724283 nm` at **`+1.49997857 σ`** of `T-71`'s measured step); what refuses it is that **`0` of `186` unique records over `30` queries reports it and `7 of 7` built cross-sections do the other thing**. So `C-0175`, `C-0180` and `C-0190` are correct about a **drawable, undemonstrated** design, and the untied readings are not the alternative either — route B is a **tether** and no lattice here carries one (`T-299`). **So this row's answer survives on the UNCOUPLED tile and its coupled half does not** **— and iteration 48 leaves that reading intact while withdrawing BOTH of the numbers it was carried on** ([`C-0205`](gpd/claims/C-0205-what-link-stiffness-the-recovery-needs.md), [`C-0208`](gpd/claims/C-0208-a-bond-link-is-two-mechanisms.md), [`CH-0258`](gpd/challenges/CH-0258-two-of-sixty-four-is-a-reading-at-a-penalty.md), [`CH-0264`](gpd/challenges/CH-0264-a-per-bond-value-against-a-uniform-threshold.md); [`C-0204`](gpd/claims/C-0204-the-anchor-azimuth-is-determined.md), [`C-0207`](gpd/claims/C-0207-the-uniform-raster-is-flat-with-its-tethers.md), [`CH-0256`](gpd/challenges/CH-0256-the-azimuth-is-not-a-convention.md), [`CH-0262`](gpd/challenges/CH-0262-the-alarm-did-not-survive-its-relocation.md)). The tied lattice's **`2` of `64`** is a reading at `RIGID_LINK_STIFFNESS = 1e4 pN/nm`, a **penalty**, and at every rung a crossover connector can supply — up to **`254.808095 pN/nm`**, against thresholds **`834.060958`** and **`607.396049`** — the census is **`0 of 64`**, the resolved per-bond link included (in plane `254.808095` exactly at 135 bonds, through the thickness `629.20588`–`1365.32644` at 300), where the tightest cell in the corpus misses `T-5b` by **`0.198 %`**. And route B's free tile is **not** past the tolerance: its span is **determined** at `0.787091706 nm` (`16 of 16` flat, `0.0569497052`–`0.0689014279` of the stroke) and its uniform rasters, graded with their own tethers, are flat at **`756 of 756`** (`0.0483790868`–`0.0946863482`) — a span being a **LEVEL** and a dishing a **FIELD**. [`CH-0265`](gpd/challenges/CH-0265-756-of-756-is-a-reading-at-the-penalty.md) was **UPHELD IN ITS PREMISE and REFUTED IN ITS CONSEQUENCE** against that last count, on the same penalty ground: re-graded at the resolved per-bond link it is `756 of 756` again, `0.048606444`–`0.0960647281` of the stroke, the link worth at most `1.02460822×` ([`C-0211`](gpd/claims/C-0211-the-uniform-raster-at-the-resolved-link.md), `T-315`). — and `10 × 6`'s *"no threshold at all"* goes with it, the enhancement-free block dishing **0.132443428** (`C-0167`) and **0.127358454** (`C-0154`, at 112 bp), neither flat. **(4c) AND THAT `0 of 64` IS A READING ON TWO *TRANSFERRED* DISTRIBUTIONS AND ON A TILE THE PROGRAMME DOES NOT RECOMMEND — SEARCHED, AND ON ROUTE B's OWN TILE, A COUPLED CELL IS FLAT *AND* ADMISSIBLE, iterations 49–50** ([`C-0212`](gpd/claims/C-0212-a-searched-distribution-at-the-resolved-link.md), `T-316`; [`C-0215`](gpd/claims/C-0215-route-b-coupled-on-its-own-stations.md), `T-322`; [`CH-0272`](gpd/challenges/CH-0272-a-flat-count-is-not-an-admissible-design.md), [`CH-0273`](gpd/challenges/CH-0273-the-buildable-ratio-window-is-a-flatness-window.md), [`CH-0276`](gpd/challenges/CH-0276-a-coupled-census-inherited-a-station-set.md), [`CH-0277`](gpd/challenges/CH-0277-a-ratio-transferred-between-two-lattices.md)). The 64 readings above are **32 cells × 2 distributions**, `C-0058`'s equal springs and its rim-graded 5:1, both *rules transferred onto* the lattice and neither an *optimum of* it. Searched instead — `C-0135`'s smoothed minimax with a multi-start descent on the true training percentile, graded **out of sample** on `C-0208`'s own 4 000-realisation stream, which it reproduces at `4.0E-9` — the same 32 cells read **`22 of 32` flat**, tightest **`0.0647024141`** against the transferred `0.100198485`, **`1.45251772×`** at that very cell. On **route B's own tile**, at stations **derived at each row length**, it is **`0 of 48`** transferred (tightest `0.100227515`) and **`27 of 48`** searched (tightest **`0.0687939715`**), of which **`7`** are also inside `C-0023`'s per-path allowable — **the first non-empty *flat AND admissible* conjunction this corpus has had**, against `3 of 32` on the `116 bp` block. **Three things travel with those counts.** No cell is inside **both** per-path flags `C-0212` reads (`12 of 32` on the ratio, `3 of 32` on the peak, `0 of 32` on both) — **and one of the two is not the threshold it is named after**, `C-0060`'s `3.5 ≤ R ≤ 20` being its own **flat** ratio window measured on a square-lattice 45-station design, with no ceiling on `R` anywhere in `C-0060` (`CH-0273`). **`0 of 48` and `0 of 32` searched cells beat the UNCOUPLED tile** at the 90th percentile (`0.0521565503`–`0.0576976711` on route B, `0.0448134881` / `0.0469005226` on the block) while `16 of 32` beat it at **zero defects** (best `0.0298112409`) — so the cost is `C-0087`'s **measured** dropout and not the coupling, and this is **not** an argument for removing it, `C-0017`'s mandate being a **placement and stability** requirement that an uncoupled tile discharges not at all (`CH-0272`). And the **station set could not be inherited**: route B's rows carry **5** station columns against the block extent's **6** at every one of the 21 phases, and the inherited ladder phase **16** refuses a five-column placement at **2 of 3** widths — the column count carrying **12 of the 27** flat cells (`CH-0276`). **(4d) AND THE PLACEMENT AND THE DISTRIBUTION ARE SYNERGISTIC, WHICH REVERSES `C-0063`'s ORDERING ON THIS LATTICE, iteration 50** ([`C-0216`](gpd/claims/C-0216-the-placement-and-the-distribution-together.md), `T-323`): the 2 × 2 reads `0.106508519` / `0.105528854` / `0.078544978` / **`0.0677344328`**, total **`−0.452630313`** log units (`1.57244277×`), **interaction `−0.1388` = `−12.96 %`, NEGATIVE**; the placement main effect is `0.009241` against the distribution's `0.3046`, a quotient of **`32.9583`**, the other way round from `C-0063`'s square-lattice reading, with the equal-spring ranking of placements *anti*-correlated with what a searched distribution wants (Spearman `−0.09354`, `−0.438018191`). **(4e) AND `15 × 4` IS NOW GRADED COUPLED IN THE SAME LATTICE STATE, iteration 51** ([`C-0218`](gpd/claims/C-0218-the-tied-regrade-at-the-other-cross-section.md), `T-294`): **`0 of 64`** at the penalty link, **`0 of 128`** at the resolved one, tightest **`0.160538609`** = `0.565012343 nm`, `1.60538609×` `T-5b`; only the tie **count** transfers (`410` bonds against `435`, ties `45 / 14` against `50 / 9`), and its free stroke is exactly **`2/3`** of `10 × 6`'s, so `T-5b` is **`1.5×` tighter in absolute nm** there and both columns must travel together. **AND THE FIRST ODD RASTER-ROW COUNT BROKE THE STANDING FALSIFIER** ([`CH-0282`](gpd/challenges/CH-0282-a-dishing-fit-assumes-an-even-raster-row-count.md), [`C-0219`](gpd/claims/C-0219-a-dishing-fit-and-the-parity-of-its-basis.md), `T-330`): a uniform pressure on `15 × 4` gives an exactly uniform field and `HoneycombDeflection` reported `0.0620506254` of the stroke as dishing, because three **independent** projections are the least-squares fit only where the corrugated gap sequence is palindromic — a **parity**, holding iff `m` is EVEN, and every block this corpus had graded has `m = 10`. Repaired the falsifier reads **`0.0`**, **15 of the 18** grillage-dishing result files are unmoved **by construction**, **3** are re-emitted, and **no verdict moves at either cross-section** — **(5) The stations exist and the placement survives them**: the honeycomb top face supplies **90** stations on `15 × 4` and **60** on `10 × 6`, at **exactly one** rooting azimuth per helix of **30°** and with **no perpendicular root anywhere** — `C-0122`'s census restored and `CH-0151`'s upward correction withdrawn (`C-0141`, `CH-0175`) — and an oblique root costs **nothing** for a flexible tie, as a symmetry, and **2.67233333×** for a crossover-hinged body (`C-0128`, corrected from 6.01719478×). Snapped onto that 21 bp ladder all sixteen placements are realisable, worst snap **3.332 nm** inside a 3.57 nm ceiling, and **three are flat — all three equal springs** ([`CH-0177`](gpd/challenges/CH-0177-the-path-count-axis-is-not-monotone.md): the rim grading loses at every count once the snap breaks the rim/interior partition it was written against), the first coupled flatness result here standing on stations a derived lattice supplies. **What is still owed**: a per-site staple-incorporation measurement on a coupling-bearing tile, which is a bench measurement and not a solve; ~~the **row length**, because a honeycomb x-raster carries **both** turn senses and has no uniform width at all (`C-0140`, `CH-0172`) — design (i)'s remedy is **112 / 108 bp**, an axial extent of **39.44 nm**, `−1.40 %` of §3's nominal, and `10 × 6` carries the same alternation with its own two lengths not yet derived~~ **RESTATED, iteration 36** (`C-0146`, `C-0147`): **the row length is derived and it is not what is owed — a CONVENTION is.** Every x-raster row of the block spans the LARGER of its two lengths exactly, at all ten rows and at every one of `C-0140`'s five candidate pairs, and the 116 bp = **39.44 nm** extent exceeds it by exactly the inter-row **STAGGER**; ~~`10 × 6` carries **the same 112 / 108 pair**, with the identical 4 / 8 bp faces and the identical extent~~ **`10 × 6` carries the same pair as `15 × 4`, and that pair MOVED in iteration 37, to `102 / 109` — see the `CH-0187` sentence below**. What is owed is **which of the two §3 names**, which is a specification question (decision **8**, `T-242`) ~~worth **six flat cells of eight against three** through a crossover column admitted by a **0.07 nm** slack (`CH-0185`)~~ **and, since iteration 38, worth NO flatness cell at all: both graded column counts belonged to a raster that does not close (`CH-0195`), and the drawable one reads the same 10 columns at every guard convention** — and the recommended cell is flat at every reading. **And the 4 bp ragged face that follows costs this row EXACTLY NOTHING** (`C-0147`): the ragged faces are the tile's **rim**, not its gap-facing surface, so the coefficient on the flatness field is exactly zero and the residual rim channel is bounded at **5.54399427e−05** and **1.68371917e−05** of the stroke ~~against **0.0274976866** of headroom, a margin of **496×**~~ **— RESTATED, iteration 45** ([`C-0175`](gpd/claims/C-0175-drawable-raster-rim.md)): at the recommended `102 / 109` raster the bound is **`9.96901722e−05`**, and that headroom belonged to a coupled cell [`C-0167`](gpd/claims/C-0167-coupled-cells-on-the-honeycomb-grillage.md) has withdrawn, so the live margins are **`262.520141×`** at the tightest zero-defect cell and **`9177.31736×`** at the uncoupled tile. **The coefficient is exactly zero at any relief, so no margin ever carried this verdict.** What it costs is **plan budget** — 1.36 nm against an outboard ceiling saturating at **2.380 nm**, **0.571** of it at 90 paths; **at the drawable raster the relief is 7 bp = 2.38 nm and the SATURATED ceiling is not reachable — the determined phase supplies 55 of 60 stations, and at 55 paths the ceiling is 3.06 nm, a margin of exactly 2 base-pair rises** (`C-0151`, read at its own census by [`CH-0202`](gpd/challenges/CH-0202-the-plan-ceiling-escape-is-quoted-at-fifty-paths.md); the claim quotes the 50-path row, 4.604 nm), and `T-258` owns the re-run of the ragged-face cost at 7 bp. ~~**And the pair itself is CONDITIONAL** (`CH-0187`): among all pairs that fit M13 the width optimum is **101 / 109 bp at `−0.55 %`**, better on three axes of four, and 112 / 108 wins only the plan budget and only at a saturated path count.~~ **`CH-0187` is ANSWERED, iteration 37, and the pair is `102 / 109`** (`C-0151`, `C-0148`, `CH-0194`): `112 / 108` needs **10** of the block's **59** raster crossovers **forced** on caDNAno's `±5 bp` scaffold rule, `101 / 109` does not close either, and the minimum stagger any DRAWABLE two-length raster can carry is **7 bp = 2.38 nm** where the filter that produced both admitted at most 4 — so the filter and the closing family are **exactly disjoint**. `102 / 109` ties at the **same 116 bp = 39.44 nm** extent, so **this row's width number does not move**; its row span is `109 bp`, its interface window **102 bp**, its columns **10** and its stations **55 of 60** at the determined phase 16. **The whole cost of closure is one crossover column** — ~~2 flat cells of eight against 3, a uniform **1.0567397–1.09611647×** of the dishing — and **the recommended cell survives at both ends of the measured band, 0.0773373597 at `f` = 0.30 and 0.0821458169 at `f` = 0.26**~~ **REVERSED, iteration 40 (`C-0167`, `CH-0213`): those are smeared-sheet readings; on the honeycomb grillage the count is 0 of 8 and the recommended cell is 0.145086839 / 0.149852804 — see item (4) above. The SELECTION of `102 / 109` is untouched, being arithmetic on the closure lattice**. **Closure is a reason to PREFER `102 / 109` and NOT a proof that `112 / 108` is unbuildable**: the rule is caDNAno's default, and `C-0152` prices the elastic half of forcing at **0.350894669 `k_BT`** per crossover, **sub-thermal**, with all ten at **0.438634952** of one crossover column of a sheet that folds. The **kinetic** half is unpriced and `C-0152` records a negative existence result over **68** queries in **7** families. Also owed: the **scaffold**, which the caDNAno paper contradicts itself about at exactly design (i) (`CH-0180`) — and there the **turn allowance is a choice**, **28 nt** against a **6 nt** reach bound, with M13 affording exactly **8** so a uniform 112 bp row fits by two nucleotides and only strained at **6.54349121–12.112167 pN** (`C-0147`). **The full derivation history — the negative this row carried for eight iterations, the four axes on which it was closed, and the two reversals since — is preserved verbatim below the table.** **AND THE WHOLE PHASE AXIS OF THIS ROW IS OVER ONE PARITY FAMILY, WHICH ONLY A DESIGN THIS REPOSITORY DID NOT DRAW COULD SHOW** ([`C-0161`](gpd/claims/C-0161-mechanics-on-an-imported-design.md) §4(b), iteration 39). `CrossoverLayout.centred` and `CrossoverLayout.phased` — the two generators every phase sweep above runs through — **alternate the column parity by construction**, and a **SEAM doubles a column pitch**, so two consecutive columns then serve the *same* interface parity. Graded on `scadnano.origami_rectangle`, the reference implementation's own Rothemund rectangle, the parity sequence is **`[0, 1, 0, 1, 1, 0]`** — **which no phase sweep in this corpus can generate**. **So every phase-swept placement, count and flatness result above is a statement about the ALTERNATING family, and a seamed sheet is outside it.** The restriction is on the **swept family**, not on the data structure: `CrossoverLayout` carries the parities explicitly and represents the seamed sequence **without complaint**, and the rectangle is representable *exactly* — **90 lattice sites against 90 drawn, 0 absent**. Nothing here re-runs those studies, and this row does **not** claim a number moves; what it claims is that the scope was never stated, because no claim could see it without a foreign design. **AND THE ONE PLACE IT COULD BITE THIS ROW IS THE RECOMMENDED BLOCK ITSELF** ([`CH-0212`](gpd/challenges/CH-0212-the-recommended-block-is-drawn-without-the-seam-its-own-claim-forces.md), raised iteration 39 and **CLOSED iteration 40 on its favourable resolution** by [`C-0168`](gpd/claims/C-0168-recommended-block-seam.md)): `C-0119` §4 derives a **forced seam** on the four-layer honeycomb tile as a tree parity and calls the 60-helix case *"a theorem, not an enumeration"*, while `C-0160`'s committed `.sc` for the same block carries **one scaffold strand with 60 domains on 60 helices** — one per helix, which is `C-0161`'s **own** seam discriminator returning *no seam* — and `HoneycombCoupledStudy` and `HoneycombPlacementStudy` **both** grade this row's cells through `CrossoverLayout.centred`. ~~**The favourable resolution is the likelier one**~~ **AND IT IS THE ONE THAT HELD** (`C-0168`): a seam needs a path graph **and** a fully folded circular scaffold, and this block leaves **919** of M13's 7 249 nt, so it closes through its own remainder — measured, the two raster termini sit **35.504 nm = 14 d** apart, both at offset 7 and on the **same** face, and the circle closes at **1.03–2.69 `k_BT`** across the whole ssDNA Kuhn bracket against the **8.0 `k_BT`** a crossover column costs; the seam would be forced only below **119–310 nt** of remainder. The path premise is false too and does not matter — the block's own cross-section adjacency carries **77 edges on 60 helices**, **18 independent cycles**, and both termini are **degree one** at every one of **27** blocks swept, so no honeycomb block of this family admits a Hamiltonian cycle at all. `CH-0212`'s **second** proposed reading could not be taken: a column parity is a property of a **staple** ladder and the emitted block carries **0** staple crossings and **0** columns. **No number moved**, both studies are inside `C-0161`'s alternating family, and the seamed counterfactual is measured anyway at **1.15701888×** (0.0240648102 → 0.0278434397), both flat. And the corpus had been quoting the **path's** seam bound, **118** domains, where this lattice's is **62** — **1.903×** too expensive. **No number moves on either resolution** — what moves is which of two standing statements about the recommended tile a reader may rely on, and it is settled by two free readings rather than a solve. **A SECOND AMBIGUITY ARRIVES WITH THE SAME DESIGN, AND IT IS A FACTOR OF TWO IN A CROSSOVER CENSUS** ([`CH-0209`](gpd/challenges/CH-0209-a-crossover-drawn-as-two-strand-crossings.md), open): the field's own generator draws **every** Rothemund crossover as **two** strand crossings at **adjacent** offsets (`o` and `o+1`) where this corpus draws **one**, so the same imported file is **90 lattice sites or 45**, and reading it as junctions softens the tile by **`1.087×`** in peak dishing over stroke (**0.258057772** as drawn against **0.28058418** as junctions, 16 duplexes, `C-0001`'s secant foundation, `T-10`'s own edge taper). `T-267` therefore **refuses to default the reading** and names it in the record, which is a mitigation and not a resolution. **None of this touches the numbers above**, which are this corpus's own designs drawn with one crossing per junction — what it touches is any verdict handed to somebody else's design, and `C-0157`'s oxDNA `k_θ` bracket, which relaxed **one** of the two motifs. |
...
**THIRD SUPERSESSION, iteration 40** ([`C-0167`](gpd/claims/C-0167-coupled-cells-on-the-honeycomb-grillage.md),
[`CH-0213`](gpd/challenges/CH-0213-the-recommended-coupled-design-does-not-survive-the-honeycomb-grillage.md)): every
**coupled** cell in the block below — and in the row above before this pass — was solved on a **smeared** single-layer
square-lattice sheet, and on the honeycomb **grillage** none of them clears `T-5b`. The **uncoupled** readings survive.
**FOURTH SUPERSESSION, iterations 43 and 45** ([`C-0180`](gpd/claims/C-0180-tied-honeycomb-coupled-regrade.md),
[`C-0187`](gpd/claims/C-0187-the-turn-prestrain-sign-is-derived.md)): the third supersession's *"none of them
clears `T-5b`"* is itself superseded — with the raster's own 59 covalent turn ties present **`2` of `64`** do at
zero prestrain, and with the turn departure's sign **derived** one global phase is left, at which **`1` of `64`**
is flat at each phase and **`0` of `64` at both**. Nothing below moves either way; the banner is again the repair.
**FIFTH SUPERSESSION, iteration 48** ([`C-0205`](gpd/claims/C-0205-what-link-stiffness-the-recovery-needs.md),
[`C-0208`](gpd/claims/C-0208-a-bond-link-is-two-mechanisms.md);
[`CH-0258`](gpd/challenges/CH-0258-two-of-sixty-four-is-a-reading-at-a-penalty.md),
[`CH-0264`](gpd/challenges/CH-0264-a-per-bond-value-against-a-uniform-threshold.md)): the fourth supersession's
`2` of `64` and `1` of `64` are readings at `RIGID_LINK_STIFFNESS = 1e4 pN/nm`, a **penalty**; the recovered cells
cross `T-5b` only at **`834.060958`** and **`607.396049 pN/nm`** where a crossover connector supplies at most
**`254.808095`**, and at the resolved per-bond link the count is **`0 of 64` at every radial rung**. And route B's
free-tile alarm is withdrawn in the same iteration
([`C-0204`](gpd/claims/C-0204-the-anchor-azimuth-is-determined.md),
[`C-0207`](gpd/claims/C-0207-the-uniform-raster-is-flat-with-its-tethers.md);
[`CH-0256`](gpd/challenges/CH-0256-the-azimuth-is-not-a-convention.md),
[`CH-0262`](gpd/challenges/CH-0262-the-alarm-did-not-survive-its-relocation.md)).
Nothing below moves either way; the banner is again the repair.
...
| Method justified against cost, cheap bound first | SCF numerics deferred twice on the stated ground that it would be *"calibrating to a guess"*, and bought only once the interaction was anchored in measurement (`T-1d`). MD declined for the Hofmeister effect because it would be *worse* than the existing measurement, not merely dearer (`C-0007`). Explicit-ion MC costed at 1–3 weeks and not run (`C-0005`). `T-15` built an in-plane *sibling* lattice rather than adding degrees of freedom to the out-of-plane one, on the ground that the two decouple exactly for a flat sheet and merging them would have forced re-verification of four published claims for a change that cannot move them. `T-19` bounded all four entry topologies by a cut-equilibrium pigeonhole before assembling a matrix, which settled two of them outright and redirected the footprint question from the lattice to the literature — where its answer was. **Iteration 46 is four instances in one pass, and in three of them the cheap bound DECIDED rather than merely ordered the work.** `T-296` re-derived a covalent **reach** bound on the measured backbone before a single query — `d − 2r_P` at `+1.49997857 σ` — which turned the question from *is it possible* into *has anyone folded it*, a literature question ([`C-0193`](gpd/claims/C-0193-the-built-turn-is-a-tether.md)). `T-252` counted the class before designing the convention the task asked for, and the count refused it: **69 of 69 deliberate, 0 stale** ([`C-0198`](gpd/claims/C-0198-a-quoted-number-has-no-link-back.md)). `T-293` asked whether its own in-scope zero was *rare* or *structural* by one pass over the corpus's **filenames** — **81** hits — before touching the predicate ([`C-0196`](gpd/claims/C-0196-a-name-cannot-govern-a-token.md)). And `T-295` priced the **static** route first and **refused it on a measurement**: several harnesses print only the first two killers per row, truncated, so the answer is not in any harness's output ([`C-0195`](gpd/claims/C-0195-the-discriminating-input.md)).
...
| Disagreement raised as a challenge, not an overwrite | ~~**One hundred and nine**~~ ~~**One hundred and fifty-four**~~ ~~**One hundred and eighty-four**~~ ~~**One hundred and ninety**~~ ~~**Two hundred and eleven**~~ ~~**Two hundred and thirty-one**~~ ~~**Two hundred and forty-seven**~~ **Two hundred and forty-six** challenges in [`gpd/challenges/`](gpd/challenges/), against ~~**one hundred**~~ ~~**one hundred and forty**~~ ~~**one hundred and sixty-one**~~ ~~**one hundred and sixty-four**~~ ~~**one hundred and eighty-four**~~ ~~**two hundred and four**~~ ~~**two hundred and fourteen**~~ **two hundred and thirteen** claims (**re-derived at this pass's baseline, iteration 45 — the corpus as committed at the end of iteration 44, with the resolved sha recorded in [`gpd/results/T-276-thirteenth-answers-synthesis.json`](gpd/results/T-276-thirteenth-answers-synthesis.json)**, by the two commands quoted below, and cross-checked against two gates that print the same numbers without being asked: [`tools/check-challenge-index.py`](tools/check-challenge-index.py) reports **211 challenge file(s), 211 indexed** and [`tools/check-corpus-identifiers.py`](tools/check-corpus-identifiers.py) reports **395 claims and challenges exist**, which is 211 + 184 exactly. **The ref is part of the number**: this pass files one challenge of its own, so its own working tree already reads **212**, and a census taken over a corpus that contains the census is not repeatable without saying when it was taken. ~~**So the row has now been stale in six of its seven passes**~~ **AND STALE AGAIN, iteration 48 — SEVEN OF EIGHT.** Re-derived at this pass's own working tree, before this claim's own files were written: `ls gpd/challenges/CH-*.md \| wc -l` returns **231** and `ls gpd/claims/C-*.md \| wc -l` returns **204**, cross-checked against the same two gates, which print the numbers on every run without being asked — [`tools/check-challenge-index.py`](tools/check-challenge-index.py) reports *231 challenge file(s), 231 indexed* and [`tools/check-corpus-identifiers.py`](tools/check-corpus-identifiers.py) reports *435 claims and challenges exist*, which is `231 + 204` exactly. **So what is missing has never been the number and is still not: it is the COMPARISON**, and the two gates that could make it are already wired. ~~**AND STALE AGAIN, iteration 52 — EIGHT OF NINE.** Re-derived at this pass's own working tree, before this claim's own files were written: `ls gpd/challenges/CH-*.md \| wc -l` returns **247** and `ls gpd/claims/C-*.md \| wc -l` returns **214**, and the same two gates print them without being asked — [`tools/check-challenge-index.py`](tools/check-challenge-index.py) reports *247 challenge file(s), 247 indexed* and [`tools/check-corpus-identifiers.py`](tools/check-corpus-identifiers.py) reports *461 claims and challenges exist*, which is `247 + 214` exactly. **This pass files one claim and two challenges of its own, so its own finished tree reads 248 / 215**~~ **WITHDRAWN, iteration 54** ([`C-0224`](gpd/claims/C-0224-a-quoted-count-against-a-pinned-record.md), `T-336`; [`CH-0292`](gpd/challenges/CH-0292-the-deliverable-quotes-the-unpinnable-half.md)): `247 / 214 / 461` and `248 / 215` are readings of an **uncommitted** tree and occur at **0** of the repository's **298** commits, so no state anybody can name produces them. That pass's own result file records the census **twice** — `selfDescribingCounts.*.atRef` = **`246`** challenges, **`213`** claims and **`459`** together at a resolved `baselineRef` of **`d7b7074`**, and the working-tree reading at no state at all — and this sentence quoted the one of the pair nothing can pin. **The pinned reading is `246` and `213`, `459` together, at `d7b7074`**, and it re-derives there for ever; the tree reading is deliberately not quoted, because a census over a corpus that contains the census is one pass behind **by construction** and the cure is to say WHICH pass rather than to read the tree — which is the previous pass's own emitted note, *"only `atRef` is emitted, and the deliverable quotes the ref rather than the tree"*. Checked from now on by [`tools/T-336-pinned-count-census.py`](tools/T-336-pinned-count-census.py), which compares what this file prints against what a result file **pins**, needs no `git` to do it, and cannot go stale when the corpus grows — a census over a corpus that contains the census is dated by when it was taken, and the derivation travels with the number here for that reason) — **more challenges than claims, and that ratio is the point** (**CORRECTED three times**: this row read *"Twenty-nine"* at iteration 4, *"Sixty-nine … against sixty"* at iteration 12 and *"Eighty-four … against seventy-four"* at iteration 16 and *"One hundred and fifty-four … against one hundred and forty"* at iteration 37 — the ratio is the invariant, not either count, and ~~**this row has now been stale five times out of six passes**~~ **six times out of seven, iteration 45**, which is the argument for deriving it rather than writing it: the counts are `ls gpd/challenges/CH-*.md \| wc -l` and `ls gpd/claims/C-*.md \| wc -l`, and ~~no retained checker reads either~~ **two gates print them under other names — see the derivation above — so what is missing is not the number but the COMPARISON**). `CH-0007` challenges **our own** queue's reading, not a subordinate's, and `CH-0021` corrects a factor `C-0014` had itself flagged as a stand-in — finding it wrong in **both** directions. |
...
- ~~**The crossover hinge constant `k_θ`.** No accessible measurement of a single-layer origami sheet's bending
  ...
  **RESTATED, iteration 40 — the vertical half is now BOUNDED rather than asserted, and it is still not MEASURED**
  ([`C-0169`](gpd/claims/C-0169-crossover-vertical-compliance.md)).
  The existing trajectory **cannot** answer it, on five checks of which the fifth is decisive: **no estimator in this
  repository computes the coordinate** — `interduplex_roll.py` computes signed *angles* and `analyse_tile.py` reduces the
  interhelical vector to its *norm* — so the 649 MB of pruned frames would not have contained the answer either, and
  `T-9`'s *"at no extra cost"* was void the moment `C-0157` was filed. Priced: `C-0157`'s own protocol plus a **six-line
  estimator**, **about one day on 8 cores** and ~649 MB retained, with the coordinate's own autocorrelation time emitted
  beside its variance. **What ran instead is the threshold, which is three orders of magnitude cheaper.** This repository
  already *derives* a crossover's in-plane **displacement** stiffness from the same two phosphate bonds,
  **64.7058824 pN/nm**, and `OrigamiGrillage`'s rigid link is a penalty **154.545455×** that — inside the one interval of
  the penalty axis `C-0099` never resolved, and the derived value sitting **1.54545455×** below `C-0009`'s own lowest convergence rung
  ([`CH-0217`](gpd/challenges/CH-0217-the-penalty-was-never-compared-against-the-crossovers-own-displacement-stiffness.md)).
  Swept alone at all 56 crossovers over four decades, the binary reading of the constraint holds on **three of four**
  pre-registered predicates: ramp fraction **0.0120392417**, peak per-crossover force moving **2.93612244 %**, flatness
  **0.0655626469** against a rigid **0.0621469105** — and it **fails the fourth by 1.12787346×**, the vertical link being
  worth **3.42** percentage points of `T-5b` (37.8530895 % → 34.4373531 % of the convention unused) against the **3.03**
  points `C-0099` measured the *entire* row-end hinge unknown to be worth. **Like it, it does not cross.** And `C-0015`'s
  registration rule survives on a better ground than a small number: its lever moves **1.18074849 → 1.17637685**, **0.370243116 %**
  softer, while with the link **deleted** every crossover carries **exactly zero** vertical force — so the rule rests on the
  link's **existence**, not on its stiffness. The **in-plane shear `k_s`** is still unmeasured, and `T-9` stays live on it.
  **AND THE VERTICAL AXIS NOW CARRIES A MEASURED TERM, iteration 48 — BUT NOT OF THE CROSSOVER**
  ...
  Resolving a bond's normal link by the bond's own direction splits it into a **transverse** (shear) and a
  **radial** (separation) mechanism, and the radial one carries the first measured term either axis of this
  problem has had: **the duplex PAIR's own `V″(d)`**, over the `21 bp` of interface one crossover owns, is
  **`205.009678 pN/nm`** from this corpus's own osmotic-stress equation of state, at a separation **above** that
  fit's data floor — and it cross-checks the fitted dihedral spring, pushing **`54.3454226 pN`** apart per
  crossover against `C-0194`'s implied **`29.7795467 pN`** inward, a ratio of **`1.82492444`** between a 2020
  measurement and a 2014 fit sharing no constant. **What is measured is the PAIR, not the CONNECTOR**: the
  crossover's own radial constant is still unsourceable, on eight further EuropePMC queries on the separation
  coordinate, one paper read directly, and two published distributions of it that are unusable for equipartition
  for stated reasons. **So this bullet moves from *bounded* to *bounded, with one of its two mechanisms
  measured on a different object* — and it does not close.**
  The registered threshold was itself mis-transcribed by **9.906×** and **both** verdicts are published
  ([`CH-0218`](gpd/challenges/CH-0218-a-pruned-trajectory-forecloses-every-question-nobody-asked-it.md) is raised on the
  pruning: a run's raw output is the only artifact from which a question nobody thought to ask while it existed can be answered).
...
- **A per-site staple-incorporation measurement on a COUPLING-BEARING tile.** ~~This is now the single most
  ...
  **SCOPE CORRECTED, iteration 45 — the superlative is struck and the measurement is not.** The closure it rests
  on is a closure about the **single-layer** tile, and the tile §3 specifies is the four-layer block, whose
  flatness answer stands on the **uncoupled** body (`C-0167`, `C-0180`) — where a per-site incorporation map is
  what carries a *coupled* cell past the tolerance and is not what carries the verdict. `DECISIONS-FOR-NDI.md`
  has said so since iteration 40 in as many words — *"it is no longer the only thing that could move the answer,
  and this paragraph said it was"* — and this bullet had not. **The measurement is still wanted, and it is no
  longer the largest single gap**: ~~the turn departure's loading channel (`CH-0240`, `T-291`) and the phase this
  section's own row (g) now records are both unpriced, and neither is a bench measurement.~~
  **CORRECTED, iteration 48 — that sentence went stale in the iteration that wrote it, and this document's own
  row (g) already said so.** [`CH-0240`](gpd/challenges/CH-0240-the-allowed-departure-is-common-mode.md) is
  **UPHELD** and `T-291` **priced** what replaces it in iteration 45
  ([`C-0190`](gpd/claims/C-0190-the-departure-is-common-mode-and-what-replaces-it.md)): `17.1428571°` of twist on
  each of 58 interior helices, graded at **`0 of 64`** flat at either sign. What is unpriced **now** is a
  different pair, and neither is a bench measurement either: the **radial** stiffness of a crossover connector
  against a relative normal displacement, which is unsourceable on eight further recorded queries
  ([`C-0208`](gpd/claims/C-0208-a-bond-link-is-two-mechanisms.md)). ~~and the link stiffness route B's own tether
  census is read at ([`CH-0265`](gpd/challenges/CH-0265-756-of-756-is-a-reading-at-the-penalty.md), **RAISED**).~~
  **The second of those was DISCHARGED in iteration 49**: route B's tether census is now read at the resolved
  per-bond link and the count does not move, `756 of 756` at every one of three resolved rungs
  ([`C-0211`](gpd/claims/C-0211-the-uniform-raster-at-the-resolved-link.md), `T-315`). The **radial** constant
  itself is what remains unpriced.
  **A bullet in this list can go stale against a row of the same document, and no retained checker reaches it:
  the sentence carries no status word and both of its identifiers resolve.**
  **AND, iteration 52, THIS BULLET HAS A PRICE FOR THE FIRST TIME — IT IS THE WHOLE DIFFERENCE BETWEEN A FLAT
  COUPLED TILE AND AN UNFLAT ONE** ([`C-0212`](gpd/claims/C-0212-a-searched-distribution-at-the-resolved-link.md),
  `T-316`; [`C-0215`](gpd/claims/C-0215-route-b-coupled-on-its-own-stations.md), `T-322`).
  With the distribution **searched** rather than transferred, **`16 of 32`** coupled cells beat the uncoupled
  tile at **zero defects** (best `0.0298112409` against `0.0448134881`) and **`0 of 32`** beat it at the 90th
  percentile of `C-0087`'s measured dropout; on route B's own tile it is `0 of 48`. So the gap this bullet names
  is not a residual uncertainty on a flat answer — **it is the term that decides the sign**, and it is still a
  bench measurement and not a solve. The measurement is wanted for a reason that is now quantified rather than
  asserted, and the sentence above — *no longer the largest single gap* — stands, because what a per-site map
  would settle is a **coupled** cell and the verdict stands on the **uncoupled** body.
- **The stiffness spread of nominally identical hybridised staple extensions.** Distinct from the incorporation
  ...
  (**Read against the queue, iteration 45**: `T-45` itself is **ANSWERED** there — *as far as published
  measurement allows*, and on the **incorporation** map, which is the bullet above and not this one. The residue
  this bullet states is what that answer leaves, and it is unchanged.)
...
- **The thermal force in a crossover.**
  It is `√(k_BT k_v)` with `k_v` the crossover's vertical stiffness, which nothing measures (`T-9`) — **though iteration 40 supplies a DERIVED value, 64.7058824 pN/nm, and shows the rigid penalty is 154.545455× it (`C-0169`, `CH-0217`)**; the
...
**AND A NINTH, iteration 48** ([`C-0210`](gpd/claims/C-0210-fourteenth-answers-synthesis.md), `T-319`, `T-320`), which had been in `DECISIONS-FOR-NDI.md` since iteration 47 **inside section 6** — a section that asks about the tile's **thickness**, so a reader looking for a **width** specification question would not have found it. It is a different question from the eighth: the eighth asks *which of two readings of one tile's width §3 names*, a **convention**, and the ninth asks whether the width that is **buildable at all** is acceptable. On the only demonstrated raster-turn topology — **28 nt** of unpaired scaffold at every turn (`C-0193`, `C-0200`) — the widest uniform honeycomb row a `10 × 6` block can carry is **`92 bp = 31.28 nm`** on NDI's own M13mp18 default and **`106 bp = 36.04 nm`** on p8064, against §3's stated **40 nm** — `−21.8 %` and `−9.9 %`, and **all three scaffold rows are flat**, uncoupled and with their own tethers graded (`756 of 756`, `C-0207`). **The tile is not too floppy; it is too small.** It is item 9 of [`DECISIONS-FOR-NDI.md`](DECISIONS-FOR-NDI.md) and `T-320`, and **its price line is left explicitly open** — `CLAUDE.md` records that when the first six were offered as yes/no, all three with a cost dimension came back in a shape the document had not offered.
...
four on `10 × 6`, best **0.0680677948**~~ **`C-0167`'s, iteration 40: every coupled cell in the block above is a
SMEARED single-layer square-lattice solve, and on the honeycomb GRILLAGE the count is `0` of `64` against `15` of
`64` smeared — while the UNCOUPLED four-layer honeycomb block stays flat, at 0.0501417315 and 0.0522223659 at the
two ends of the measured band.** The block is kept as the record of what iteration 23 established.
**RESTATED AGAIN, iteration 43** ([`C-0180`](gpd/claims/C-0180-tied-honeycomb-coupled-regrade.md),
[`CH-0234`](gpd/challenges/CH-0234-no-verdict-reverses-was-a-free-tile-statement.md)): that lattice omits the
raster's own **59 covalent turn ties**, and with them present the count is **`2` of `64`** — the uncoupled block
staying flat and moving the favourable way, to **0.0446459684** and **0.0467367262**.
The recovery is **0.426 %** of the tolerance at its tighter cell and ~~rests on a prestrain **sign nothing here fixes**
(`CH-0228`, [`T-284`](TASKS.md)), so the count is not settled.~~
**RESTATED, iteration 45** ([`C-0187`](gpd/claims/C-0187-the-turn-prestrain-sign-is-derived.md),
[`CH-0240`](gpd/challenges/CH-0240-the-allowed-departure-is-common-mode.md)): **the sign is derived** — caDNAno's own
`±5 bp` rule pins **58 of the 59** binaries, strictly alternating — so what is left is one **global phase**, at which
**`1` of `64`** cells is flat at each phase and **`0` of `64` at both**. The count is **phase-contingent**, not
unsettled in its sign; and the relative-roll coordinate the load is applied through has coefficient **exactly zero**
for a level displacement (~~`CH-0240`, raised~~ [`CH-0240`](gpd/challenges/CH-0240-the-allowed-departure-is-common-mode.md) is **UPHELD**, iteration 45 ([`C-0190`](gpd/claims/C-0190-the-departure-is-common-mode-and-what-replaces-it.md), `T-291`)), ~~which is `T-291`~~ **and `T-291` has priced what replaces it**.
**`CH-0240` is now UPHELD** ([`C-0190`](gpd/claims/C-0190-the-departure-is-common-mode-and-what-replaces-it.md),
`T-291`), and the replacement — `17.1428571°` of **twist** on 58 interior helices — is graded at **`0 of 64`**
...
([`C-0205`](gpd/claims/C-0205-what-link-stiffness-the-recovery-needs.md), `T-303`;
[`C-0208`](gpd/claims/C-0208-a-bond-link-is-two-mechanisms.md), `T-310`): the counts above are read at
`RIGID_LINK_STIFFNESS = 1e4 pN/nm`, a **penalty**, and the two recovered cells cross `T-5b` only at
**`834.060958`** and **`607.396049 pN/nm`** against a connector ceiling of **`254.808095`** — **`0 of 64` at every
rung of the resolved per-bond link**, the tightest cell missing `T-5b` by **`0.198 %`**.
