...
1. **The tile is not a rigid plate, and the picture has to go.** It is rigid *exactly* under a uniform load —
   ...
   **And the coordinate itself is now disputed** (`CH-0240`, raised): a level displacement rotates **both** backbones
...
for a level displacement (`CH-0240`, raised), which is `T-291`.
