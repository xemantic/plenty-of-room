...
1. **The tile is not a rigid plate, and the picture has to go.** It is rigid *exactly* under a uniform load —
   ...
   **0.0577199433** of the stroke under the same solved collar **with no attachment coupling at all**, inside
   ...
   **AND THE LINE THEN RAN THREE STEPS FURTHER, ALL FAVOURABLE** (iteration 25). The tile is **design (i)
   ...
   flatter and has NO threshold at all**, flat even at `f = 0` where the layers do not couple, so it does
   not depend on the calibration the `15 × 4` verdict rests on — at **two-thirds of the footprint**, a
   specification trade rather than a free improvement (`C-0120`). And **a COUPLED tile is flat under the
   ...
   **UNDER REVISION, iteration 33 — do not read the four-layer numbers above as settled** (`C-0141`, [`CH-0174`](gpd/challenges/CH-0174-the-four-layer-cross-section-is-not-a-honeycomb.md), queued as `T-233`). **The cross-section every four-layer claim in this corpus is written on is not a honeycomb**: a honeycomb spends `3√3/4·d²` of plan per helix and the standing model spends `d²`, so every four-layer `edgeY` is exactly **1.5×** too small. Corrected, the **footprint ordering reverses** — `15 × 4` is 1.40084263 of §3's 40.35 nm and `10 × 6` is 0.929467162, so `10 × 6` carries the ~38 nm the passages above attribute to `15 × 4` — and `C-0116`'s interlayer threshold moves to **0.276970522**, **inside** the measured 0.26–0.33 band, where `15 × 4` dishes 0.101759944 and **fails** `T-5b`. `10 × 6` dishes 0.0240648102 and stays flat across the whole band. **The four-layer verdict is therefore conditional in a way this passage does not yet say, and the cross-section that survives is `10 × 6`** — already `DECISIONS-FOR-NDI.md`'s seventh decision. Restated at `T-233`, not here.
...
**Six were sent and answered on 2026-08-18; a SEVENTH was raised in iteration 26 and is the first this programme has raised against its own recommendation.** It is item 7 of [`DECISIONS-FOR-NDI.md`](DECISIONS-FOR-NDI.md) and item 12 of [`TASKS.md`](TASKS.md)'s register, recorded in both at the same time — because the last time this set gained a member, the register never learned about it.
