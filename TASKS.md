...
| P-12 | Fold the multi-agent harness workarounds into `tools/` | A `--drop <pkg>` flag on `verify.sh`, and a `tools/study.sh` that runs a study on an isolated copy | TODO — low. Raised by `C-0015`, which lost a study run to a **third** cause of `NoClassDefFoundError`: neither the build race nor an OOM worker, but a **sibling package another agent left mid-TDD**, whose failed `compileKotlin` leaves `build-<agent>/classes/kotlin/main` half-written. |
