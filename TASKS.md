...
| T-23 | A **two-sided** compliant DNA coupling — an antagonistic spacer pair, or a bending hinge in place of a stretched chain | An element that carries load in both directions at ≤ 40 pN/nm, or a demonstration that DNA offers none | A8.2 | TODO — **medium-high**. `C-0021` shows `T-16`'s stiffness margin and `T-13`'s hold-down are **one design variable** (`F_down = (k_c − 33.333)·3 nm`), and that only a two-sided element can spend it. A coupling 4 % above §3's mandate would supply the entire thermal-scale hold-down and remove the tethers from the design. Cheap: the elements are all in `anchoring/`. |
