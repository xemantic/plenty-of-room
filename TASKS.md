...
| P-15 | Repair `bracketedRoot`'s Illinois sign test | TODO — **medium, and it is a correctness fix, not a cleanup** | Raised by `C-0019` (`S-143`), which observed `bracketedRoot` evaluating its function **outside its own bracket**: `atLeft * atEstimate < 0.0` underflows to `−0.0` when both factors are tiny, the stagnant endpoint is replaced by one of the same sign, and the next secant step escapes. It bites when the residual spans decades. **Not repaired inside `T-1f` because `C-0003`, `C-0011` and `C-0016` all consume it** and their result files must be re-run and diffed as part of the fix. |
...
| T-40 | Is a duplex standing normal to a single-layer sheet buildable, and what holds its base? | The base joint of `C-0025`'s normal standoff, modelled as one of its own motifs, against the clamp it assumes | A8.2 | TODO — **high, and it is the single assumption `C-0025`'s design rests on**. The standoff is the only joint in the catalogue that passes, and its own rotational stiffness assumes a clamped base. If the base is a crossover (`k_θ = 13.53 pN·nm/rad`) the standoff is a pinned column and its transverse support and buckling load both change. No origami literature on out-of-plane duplexes was consulted. |
| T-41 | The cantilever joint's off-diagonal compliance | `c` and `S_eff` re-derived with the joint as a 2 × 2 flexibility matrix rather than two independent springs | A8.2 | TODO — medium. `C-0025` treats `k_θ` and `k_a` as independent; a real cantilever standoff has `δ = Fℓ³/3EI + Mℓ²/2EI`, so its end rotation and axial motion are coupled. It **softens** the joint, so the compliance verdict is conservative — but the window's lower edge (the 40 pN/nm ceiling) is exactly where a softening correction would move it. Cheap: one 2 × 2 solve. |
