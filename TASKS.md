...
| P-5 | Decide and defend the brush-regime criterion (`Σ ≥ 5` vs `Σ > 1`) | **RE-OPENED** (iteration 3) — medium | **`L₀/R₀ ≥ 1` cannot bound a window from below** once `L₀` is an *onset* height: it then measures how far into the coil's tail the threshold sits and cannot fall below one — it passes a layer at `Σ = 0.10`, a carpet of isolated mushrooms. Coil overlap `Σ = πR₀²σ ≥ 1` must be carried alongside, and it is also the 1-D SCF's own validity condition (`C-0011`). Any window edge resting on `L₀/R₀` alone is unbounded below. Earlier adoption by `T-1c`: the criterion is **`L₀/R₀ ≥ 1`**, reported as a number at every design point, with windows emitted both with and without it so the two contributions stay separable. `Σ ≥ 5` is dropped — it failed thermodynamically (`CH-0001`) and geometrically (`CH-0003`). |
...
| T-2 | Feasible design window in (grafting density, height, chemistry) | Non-empty region satisfying §4(a)–(d) simultaneously, or a proof of emptiness naming the binding constraint | A2.1 | **UNBLOCKED by T-1d, and BOTH branches are now available. HIGH — this is where the programme turns.** It may conclude **non-empty at 10 nm** (`σ ∈ [0.0116, 0.2601] nm⁻²`) **and at 7 nm** (`[0.0296, 0.0496]`), **empty at 5 nm**, and the ~10 nm desired stroke unreachable everywhere. It **must state which layer-height convention its window is in** — force-onset or first-moment — because the polymer to order differs by ~4× between them. Constraints with no axis yet: output coupling ≳ 64 attachments (`C-0009`), 5–72 pN/nm of coupling stiffness (`C-0012`), a frame standoff scaling with stroke (`C-0014`), and a usable bias window of 0.02–0.1 V (`C-0012`). Superseded note — was: **UNBLOCKED by T-1c**, and its deliverable had changed shape: at 10 nm **neither branch of the predicate is currently available** — the window's existence is decided by the *profile model*, not the interaction, so "non-empty region" and "proof of emptiness" are both unsupported until `T-1d`. What **is** robust across all six models: 5 nm and 7 nm are **empty**, and the ~10 nm desired stroke is **unreachable everywhere**. Inputs: §4(d) discharged as a non-constraint and tile edge bounded above at 437 nm (`C-0004`); §4(c) answered with the opposite sign (`C-0005`); and a **topological** constraint with no axis in the current window — the output coupling needs ≳ 55 load paths against 43.7 independent patches (`C-0006`). |
...
| T-4 | Electrostatic softening and pull-in: does `k_eff = k_brush + k_es` reach zero? | Max usable bias with margin, or a demonstration the osmotic divergence removes the instability | new | TODO — **HIGH, and RE-FORMULATED by `C-0012`.** The question is no longer *whether* `k_eff` reaches zero — **it does, at the predicate's own operating point**, crossing between 0.05 and 0.10 V at 10 nm and between 0.10 and 0.25 V at 7 nm, **an order of magnitude below** `C-0008`'s resting-height estimate. The deliverable becomes the **maximum usable bias, which upstream validity already caps at ~0.1 V**. And `CH-0011` says §6 task 4's second branch is nearly right but for the wrong reason: the collapse **is arrested**, but by `k_es` reversing sign at 0.55–1.58 nm, not by the osmotic divergence. Superseded note — was: **PROMOTED to high; now the most likely binding constraint in the programme.** `C-0008` gives `k_es` directly from the solve, so §1's `\|k_es\| ≈ F_es/λ_D` must not be inherited (it understates by 1.0–2.6×, non-conservative for pull-in). Against `C-0001`, `\|k_es\|` reaches `k_brush` at **0.158 / 0.098 / 0.077 V** (5 / 7 / 10 nm, 2 mM) — and set against the bias needed for 100 pN the margin **inverts** across the height range: 2.4× at 5 nm, 0.87× at 7 nm, **0.11× at 10 nm**. Blocked only by `T-3`. |
...
| T-3b | 2-D Poisson-Boltzmann solve of the tile edge | The lateral load non-uniformity `T-5b` needs, as a profile rather than a parameter | new | TODO — **promoted to high** by `C-0012`: it now decides whether the lever/sensor split is 11 % or 369 % of a stroke that is itself well determined. Was: **medium**. The only route to closing §4(g): `C-0006` makes dishing *exactly linear* in the load non-uniformity, and `C-0008` states plainly that a 1-D treatment cannot supply it. Hours, not days. |
...
| T-1f | Bound the mean-field fluctuation corrections at `φ ≈ 0.01` | The correction bounded, or declared unbounded with the missing method named | A2.1 | TODO — **medium-high**. Now **the largest unbounded exposure** under `C-0011`, which says plainly that it does not bound them: the layer sits at `φ ≈ 0.01`, which is where mean field is furthest from safe. |
| T-16 | **Minimum output-coupling stiffness** — the lever must supply its own stiffness where `k_eff < 0` | The coupling stiffness needed to hold the operating point, against what a DNA-origami lever can deliver | new | TODO — **medium-high**. `C-0012` supplies the number: **5.3–16.0 pN/nm at 10 nm / 0.10 V, rising to 47.6–71.5 pN/nm at 0.25 V** — comparable to the whole layer at first contact. It pairs with `C-0006`'s topological constraint, and `T-2`'s window has no axis for either. |
| T-15 | The in-plane (membrane) load path into the tile, by shear lag | The in-plane force-concentration factor, replacing `C-0009`'s out-of-plane one used as a bound | A8.2 | TODO — **medium-high**. `C-0014` had to apply `C-0009`'s 2.3–7.6× out-of-plane concentration to an in-plane load as a conservative stand-in. If the in-plane factor is ~1, **every minimum tether length shrinks by up to 2.8× and §3's desired 10 nm stroke becomes reachable at a 34 nm tether**. Hours on the existing grillage. |
| T-13 | Where the tile sits at zero bias | Whatever holds the tile down, named and quantified | new | TODO — **re-scoped and promoted** by `C-0014`, which partly answered it as a by-product: a taut entropic tether supplies **4.6–9.4 pN of downward preload with the same element that confines laterally**. `T-13` should evaluate that before inventing anything. Was: medium. A non-adsorbing layer exerts **no upward force above `L₀`**, and three of six `C-0003` models have exactly zero stiffness at `L₀`, so the unbiased tile is unconfined in *both* directions. Nothing in the §3 stack owns this and no task in the programme currently does either. |
...
- **No discrete attachment scheme is flat**, and the lattice-native form of the statement is stronger than the
  continuum one. Solved on *both* models, flatness needs **64 attachment points against the tile's 56
  crossovers** — 1.14 attachments per across-helix load path, i.e. **more attachments than the tile has load
  paths**. `C-0006`'s "55 against 43.7 patches" was a continuum heuristic and 14 % optimistic. The count is
  pure lattice geometry, so it does not move when the layer stiffness is re-derived. The output coupling has
  to be effectively continuous, and `T-2`'s window has no axis for it. (`C-0006`, `C-0009`.)
- **A load-path contour average is not a peak.** A rigid anchor is carried by its **two nearest crossovers and
  ...
  in the sweep, **11.54 pN on one crossover, reaches the 10–15 pN single-duplex unzip allowable** — a
  discretely anchored tile is *not* comfortably safe. (`C-0009`.)
