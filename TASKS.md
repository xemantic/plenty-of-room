...
| T-16 | **Minimum output-coupling stiffness** — what a DNA-origami lever can actually deliver | The coupling stiffness a lever can supply at the tile, against the 5–277 pN/nm the operating point demands | new | TODO — **HIGHEST PRIORITY IN THE PROGRAMME**, and cheap. `C-0016` makes this **the single number that decides whether Gen-1 has a design window at all**: at 7 and 10 nm the §6 operating point is statically unstable under *every* layer model at §3's own 2 mM buffer, so it exists only against a lever supplying its own stiffness. **If no lever reaches 5–277 pN/nm, P2 closes empty at 7 and 10 nm and P1 has already emptied 5 nm — no window anywhere.** |
