...
| T-19 | The attachment's entry topology — what a tether actually bonds to | The peak per-load-path force for a tether bonded across two duplexes, or onto a crossover, against the one-point-on-one-duplex model | A8.2 | TODO — **medium, and cheap (hours on the `T-15` membrane)**. `C-0020`'s headline is `η = 1` *because* a tether loads one point of one duplex, so the attachment is the most loaded member by construction. Bonding across two duplexes would halve it and take `L_min(10 nm)` from 33.5 to ~24 nm; presenting the joint in **unzip** geometry raises it 2.2×. This is the single modelling choice the whole in-plane result rests on, and it is a sequence-design choice, not physics. |
...
| T-13 | Where the tile sits at zero bias | Whatever holds the tile down, named and quantified | new | TODO — **re-scoped and promoted** by `C-0014`, which partly answered it as a by-product: a taut entropic tether supplies **4.6–9.4 pN of downward preload with the same element that confines laterally**. `T-13` should evaluate that before inventing anything. `C-0020` hands it a second term: a minimum-length **in-plane** tether supplies **54.9 pN of downward preload, independent of the stroke** — 55 % of the §3 target force — where `C-0014`'s entropic tether supplied 4.6–9.4 pN. Was: medium. A non-adsorbing layer exerts **no upward force above `L₀`**, and three of six `C-0003` models have exactly zero stiffness at `L₀`, so the unbiased tile is unconfined in *both* directions. Nothing in the §3 stack owns this and no task in the programme currently does either. |
