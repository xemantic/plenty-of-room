...
| **Status** | **UPHELD as a bookkeeping correction, while the recommendation it challenges STANDS**, by [`C-0103`](../claims/C-0103-path-count-at-fixed-geometry.md) (`T-163`) — over 21 graded cells the missing column is real and it is **small at 34 → 30**, so the trade this challenge names exists and does not reverse the design. **What that claim additionally withdraws is this challenge's own instrument**: the monotonicity it quotes is read through *placement-searched* subsets, and a scaling exponent fitted through per-point optima is not an exponent in the swept variable, which is [`CH-0119`](CH-0119-a-redundancy-slope-measured-over-searched-subsets-is-not-a-count-slope.md). No headline of `C-0072`, `C-0074` or `C-0075` moves |
