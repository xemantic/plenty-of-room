...
| **Status** | ~~**raised.**~~ **ANSWERED** by [`C-0148`](../claims/C-0148-face-bond-class-residues-and-row-span-columns.md) (`T-243`) — **the twelfth column is a bounding-box artefact**: a crossover column serves an *interface*, whose window is the intersection of two row spans and never the union of every row, and on that window the count is **11** at all three `EDGE_MARGIN` conventions. The `EDGE_MARGIN` KDoc is replaced by the condition rather than by a tile width. **The price this challenge carried — six flat coupled cells of eight against three — is therefore withdrawn**, which is what `DECISIONS-FOR-NDI` decision 8 rested on. The **grounds** stand: this is the second geometry at which the inertness sentence fails — [`C-0134`](../claims/C-0134-buildable-width-count-phase.md) found it false at 38.08 nm on the square lattice, where it deleted two of eight columns at exactly the two phases the design wanted |
