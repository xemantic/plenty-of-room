...
| **Against** | [`C-0059`](../claims/C-0059-torsion-feasible-routing.md)'s base misalignment floors and [`C-0062`](../claims/C-0062-crossbar-trio-existence.md)'s design table, which composes them with its own cap floors — specifically the **6.0°** base floor at the 9 and 10 bp rows and the *"best representable design is the 10 bp row at both ends"* verdict that rests on it |
...
| **Status** | **UPHELD** by [`C-0070`](../claims/C-0070-pinned-leg-budget.md) (`T-132`) — *"`CH-0078` is upheld and nothing softens it"* — which composes the pinned station with a shared leg length and finds the 10 bp row `C-0062` recommends **not representable at any** shared length, the 7 and 8 bp rows likewise gone, and the verdict the challenge predicted is the one it gets. **This challenge's own *what would settle it* item 2 is DISCHARGED.** The pinned composition is marginally **better** than the free one at every representable row, and no upstream number fails to reproduce |
