...
| **Status** | **DISCHARGED in the item it left unevaluated** by [`C-0090`](../claims/C-0090-buildable-raster-width.md) (`T-153`) — the number of upward stations, the crossover-phase census and every plan margin are evaluated there, and this challenge's own sign guess is **corrected**: it calls the effect *"unfavourable but small"* and the flatness moves **favourably**. The width statement itself is not adjudicated by that claim and **STANDS as a statement about a SEAMLESS raster**, which is what `C-0086` recommends. It does **not** assert that 40.0 nm is unbuildable in general: the seamed case is not derived, and Rothemund himself offers an escape (*"one or two scaffold bases could be left unpaired"*) whose cost he introduced it to avoid |
