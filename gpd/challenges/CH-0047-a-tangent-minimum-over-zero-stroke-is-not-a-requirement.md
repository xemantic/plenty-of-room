...
| **Status** | **ANSWERED by [`C-0049`](../claims/C-0049-compliance-ceiling-stroke.md) (`T-107`), 2026-08-14 — the convention is `[0, s*]`, the strokes the device TRAVERSES, and it moves a verdict after all.** Was: *"OPEN, and narrow. No verdict in `C-0030` or `C-0032` moves — the softening element's minimum is interior and identical on both ranges. What moves is every comparison against a strain-stiffening element."* On the traversed range `C-0030`'s softening element's minimum sits at its **upper** endpoint, the placement stroke, where it is **25.227 pN/nm** and clears **4 of `C-0017`'s 6** model floors at 10 nm / 2 mM — against **0 of 6** for the interior 22.875 at 4.556 nm, which a device placed at 3 nm never reaches. `C-0032`'s `Q2` therefore goes from *"fails at 6 of 6"* to *"fails at 2 of 6"*; its `Q3` and its 1.0000–1.0019 bias margin are untouched, so §3's 2 mM is **not** recovered. **It is the same question as `T-107`'s ceiling** — a requirement is owed over the strokes the device traverses, whichever side of the inequality it is on |
