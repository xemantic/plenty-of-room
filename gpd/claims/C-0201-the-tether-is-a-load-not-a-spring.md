# C-0201 — **ROUTE B'S RASTER TURN IS A LOAD, NOT A SPRING, AND IT IS A DISHING *SOURCE* WHERE ROUTE A'S TIE IS A *SINK*.** The tether's own stiffness is `0.22377084–0.919840405 pN/nm` — at most **`0.109313066`** of `k_θ` on the rim node's `d/2` arm and **`9.19840405E−5`** of the link penalty the lattice uses — so with its preload removed it moves the free tile `0.0501417315 → 0.0496660245`, a **`0.95 %`** move *toward* flatness. **What moves the tile is the chain's own tension**, `0.160569993–3.03288672 pN`, a self-equilibrated internal load in `C-0104`'s exact sense: no entry of the stiffness matrix moves, the field is **linear** in it, and its projection is `f·unitZ` on the link gradient, so the **nine in-plane turns contribute exactly zero** and the **fifty through-thickness ones** carry it. With the preload the free tile reads **`0.11296458`**, past `T-5b`'s `0.10`. Graded on `C-0167`'s own 64 coupled cells, the same stations, the same distributions and the same 4 000-realisation stream, **all four tethered states read `0 of 64` flat at the 90th percentile against `C-0180`'s tied `2 of 64`, the tether is a dishing source at `64 of 64` cells at the built allowance and `248 of 256` over all four tethered states** (median per-realisation ratio `1.0046118–1.76745293` at the built allowance), and **the verdict does not move over four decades of `k_link`**. **The FREE tile straddles the tolerance on an AZIMUTH convention nobody has measured** — `0.0569815008` at the aligned azimuth and the softest chain against `0.166312182` at the worst and the stiffest, `24 of 36` bracket corners flat. `CH-0251` is **REFUTED** by `C-0200` this same iteration, so **this is the arm the built object occupies** and `C-0175` §9's `1.12×`, `C-0180` §4's `2 of 64` and `C-0190`'s `17.1428571°` are all about the design nobody has folded. And the width route B forces is the larger finding: at the built allowance a uniform honeycomb row is **`92 bp = 31.28 nm`** on M13mp18 and **`106 bp = 36.04 nm`** on p8064 against §3's `40 nm` — flat, and too small — **CORRECTED, iteration 48 ([`C-0204`](C-0204-the-anchor-azimuth-is-determined.md) / `T-304`, [`CH-0256`](../challenges/CH-0256-the-azimuth-is-not-a-convention.md)): THE AZIMUTH IS NOT A CONVENTION AND IT IS NOT UNMEASURED — IT IS DETERMINED BY THE VERY RASTER THIS CLAIM GRADES.** On the drawable `102 / 109` raster every one of the 59 turns anchors at an *allowed* scaffold crossover, so the span is **`0.787091706 nm`** — `C-0152`'s own number — at all 59, and the free tile is **flat at `16 of 16`** surviving corners, `0.0569497052–0.0689014279`. **The `0.11296458` above is the `d + 2r_P` corner, which this raster reaches at NO turn**, and *"the free tile straddles the tolerance"* is withdrawn for this design. Everything else here stands and is reproduced by `C-0204`: the element, the preload, *load not a spring*, *source not a sink*, the `0 of 64`, and the `k_link` ladder. §9's own naming of this gap is what `T-304` executed — and ~~the alarm is **relocated** rather than withdrawn, to route B's own uniform rasters~~ — **WITHDRAWN IN TURN, iteration 48 ([`C-0207`](C-0207-the-uniform-raster-is-flat-with-its-tethers.md) / `T-307`, [`CH-0262`](../challenges/CH-0262-the-alarm-did-not-survive-its-relocation.md)): graded with their tethers those uniform rows read `0.0483790868–0.0946863482` of the stroke, `756 of 756` cells flat at every lattice phase and every chain corner, and §7's three untied readings are reproduced there at `8.6e−10`.** The span distribution is real and reproduced — worst turn `3.93454333–4.35327572 nm` at every one of the 21 lattice phases (`T-307`) — and it does not cost this tile its flatness. §7's larger finding, that the tile is too **small**, is untouched
...
> **CORRECTED, iteration 48 (`C-0204` / `T-304`, `CH-0256`).** The three rows of the table below that are read at the **worst** azimuth — including the
> `0.11296458` this claim headlines — are at a state the graded design does not occupy. The span is **determined** at `0.787091706 nm`
> and the collapsed bracket is flat at `16 of 16` corners, `0.0569497052–0.0689014279`. The three values below are **reproduced** by
> `C-0204` at departures of `6.7e−10`, `1.9e−10` and `3.6e−10`, so nothing here is withdrawn as arithmetic; what is withdrawn is
> reading the worst bracket end as the design's own.
...
[`tools/T-299-mutation-test.py`](../../tools/T-299-mutation-test.py) (moved there by `T-305`).
