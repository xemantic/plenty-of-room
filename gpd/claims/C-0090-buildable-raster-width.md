...
- **The end-of-row convention is a MODELLING CHOICE and both readings are carried.** The physical argument for admitting it is that a seamless boustrophedon's row-end scaffold crossover certainly exists; the argument for refusing it is that no crossover has ever been drawn at the last base pair of a duplex and the grillage cannot represent one without an inset. **The verdict differs between them** — **CORRECTED, iteration 19 (`C-0095`, `T-161`): the first half of that refusal is FALSE.** One has been drawn: Rothemund's **24-helix rectangle is 288 bp = 18 column pitches exactly**, so both vertical edges lie on the crossover lattice, and it folded **90 % well-formed** — in the paper this programme's lattice comes from. Nor does the software forbid it (caDNAno *"permits the user to force crossovers between any two scaffold bases"*; cadnano 2.1 **automates** the raster turn), nor does the geometry: a crossover is an **azimuthal** condition and *"the last base pair"* an **axial** coordinate. Stronger still, `C-0095` shows the odd-half-turn rule and the row-end column parity condition are the **same congruence**, so the row-end column **is** the raster turn as a theorem rather than an argument. **The programme carries the ADMITTED reading, 0.0621469105**, and the second half of the refusal survives only as a *stiffness* question — how stiff a row-end crossover is, which is `T-164`/[`CH-0111`](../challenges/CH-0111-the-row-end-crossover-is-admitted-with-an-interior-crossover-s-stiffness.md) (0.0621 against 0.1684) and that is stated, not averaged.
...
2. **Which end-of-row convention is right.** It is a question about whether a crossover can be drawn at the last base pair of a duplex, and it is worth the whole verdict (0.0621 against 0.1684). **This is a design-language question, not a modelling one, and it should be put to NDI beside the scaffold question `C-0086` already carries.** **CLOSED TWICE: on the permission by `C-0095` (iteration 19) and on the mechanics by [`C-0099`](C-0099-row-end-crossover-stiffness.md) (iteration 20).** The 2.70925468× between the two readings is **97.40 % the vertical link** — a covalent constraint, not a stiffness — 2.85 % the dihedral spring and −0.25 % the mesh node, and softening the spring to **zero** leaves the tile at 0.0651753854, inside `T-5b`. **No NDI question is added** ([`CH-0115`](../challenges/CH-0115-the-row-end-bracket-is-a-constraint-not-a-stiffness.md)).
