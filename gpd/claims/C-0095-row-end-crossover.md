...
1. **How stiff is a row-end crossover?** `CH-0111`. It ties two duplex **ends**, and Rothemund says its strain is unrelieved and unresolved. The bracket is already computed (0.0621 to 0.1684); what is missing is where inside it the answer sits. **This is a modelling task (an oxDNA or all-atom edge crossover, or a `k_θ` sweep on the two end columns), not a specification question.** **ANSWERED, iteration 20 ([`C-0099`](C-0099-row-end-crossover-stiffness.md), `T-164`): it does not matter.** The `k_θ` sweep was run — destroying the dihedral spring of all 14 takes the best 34-root placement 0.0621469105 → **0.0651753854**, still inside `T-5b`'s 0.10 with **34.8 %** of the convention unused — and **the bracket is not a bracket**: 97.40 % of it is the vertical **link**, which is a constraint expressing the covalent continuity this claim itself establishes, and only 2.85 % is the spring. The oxDNA edge crossover is **recommended against** ([`CH-0115`](../challenges/CH-0115-the-row-end-bracket-is-a-constraint-not-a-stiffness.md)).
...
4. **Whether the dishing is monotone in the row-end crossover's stiffness.** Assumed nowhere and used nowhere, but it is what would let the bracket be read as an interval. **MEASURED, iteration 20 (`C-0099`): it IS monotone**, at 16 of 16 consecutive pairs over two channels — and no variational argument would have given it, a peak dishing being not an energy and then minimised over 163 296 placements.
