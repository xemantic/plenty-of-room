...
| **Provenance** | `gpd/results/T-17-one-row-per-duplex.json`, produced by `coupling.OneRowPerDuplexStudyKt`; model in `origami-engine/src/main/kotlin/coupling/UniformityBudget.kt`; **22 gate-named tests** in `origami-engine/src/test/kotlin/coupling/UniformityBudgetTest.kt` (61 in `coupling`, **931 in the suite, 0 failures** on `tools/verify.sh`). 15 grid shapes × 6 load profiles × 1–5 foundation stiffnesses = **133 solved lattice-and-plate states**, plus a **23-profile sweep** on the design grid, 20 scatter states, 10 thermal states, 15 duty records, 6 allowable crossings, 8 convergence records and 7 upstream reproductions. The result file re-run through `tools/study.sh` and diffed **byte-for-byte identical**. |
...
| **Challenged by** | [`CH-0073`](../challenges/CH-0073-the-along-helix-scatter-rule-reverses-on-a-non-uniform-coupling.md) (2026-08-14), from [`C-0060`](C-0060-buildable-stiffness-ratio.md) (`T-122`) — against the **build rule** *"if they must differ, let the error be along the helix"*, on the ground that the exact zero is a symmetry of **equal springs on one row per duplex** and `C-0058`'s non-uniform coupling does not have it. **No number here moves**: the 0.883 pN per unit amplitude, the `3e−11 pN` exact zero and the 17 % break-even are all reproduced or cited unchanged, and the rule survives on the **crossover-force** channel. What is added is a second channel that ranks the two patterns **oppositely** — on `C-0058`'s three-column design the along-helix index *is* the rim/interior index, and the flatness verdict is lost at **31.6 %** there against **69.8 %** across the helices |
...
Executed as **22 gate-named tests** in `origami-engine/src/test/kotlin/coupling/UniformityBudgetTest.kt`, all green.
