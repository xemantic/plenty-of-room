...
**The LEFTMOST verdict is the live one.** **12** rows carry a leading `TODO` and are read CLOSED by the old reader, and only **3** of them are the defect; the other **9** (`T-263`, `T-265`, `T-266`, `T-267`, `T-270`, `T-271`, `T-274`, `T-275`, `T-276`) carry a live `**DONE**` in an **earlier** cell and preserve the original `TODO — **PRIORITY**` note in a **later** one — ~~`C-0071`'s *strike, never delete* applied to a whole column~~ **[`CH-0241`](../challenges/CH-0241-the-preserved-priority-idiom-is-a-dropped-cell.md) (iteration 44): that ground is WITHDRAWN and the rule is UPHELD.** There is no *whole column*: read at this claim's own baseline ref with a predicate that locates each table's status column from that table's own header, **9 of those 9 rows carry their verdict outside it**, because they have **dropped the `Leaf` cell** — so the record renders under `Leaf` and the preserved note lands in the status cell behind it. `tools/check-markdown-tables.py` is clean on all nine, because their cell **count** matches the header and only the **meaning** does not. Taking the leftmost verdict is still right, on the **measurement** below rather than on an idiom; what the withdrawn ground concealed is **when it fails** — `T-276` is the tenth row of the same shape with the two contents exchanged, and there the leftmost verdict is the **superseded** one and a live **HIGH** row left the register ([`C-0188`](C-0188-a-verdict-in-the-wrong-column.md), `T-289`). So *"any leading `TODO` opens the row"* and *"the last verdict wins"* are both **9 false positives**; taking the **first** reproduces the old reader on **262 of 262** rows that carry a verdict, and the remaining **10** carry none and agree with the fallback at **10 of 10**.
