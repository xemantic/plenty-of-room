...
| **Provenance** | `gpd/results/T-208-result-file-hygiene.json`, emitted by `tools/T-208-emit-result.py`; the new checker `tools/check-result-file-hygiene.py` (**47** self-tests, three mutations each failing a named one); `DEPARTURE_SIGNIFICANT_DIGITS`/`DEPARTURE_DIGITS_BY_KEY` in `origami-engine/src/main/kotlin/structure/ResultRounding.kt` with **4** gate-named tests; `saturatedProportionBound` in `origami-engine/src/main/kotlin/coupling/StapleDropout.kt` with **5** gate-named tests; `anchoring.TwoPerRowPlacementStudyKt` run **twice** and `coupling.StapleDropoutStudyKt` once through `tools/study.sh`; the diff baseline is `git HEAD:gpd/results/…` in both cases |
