...
| **Provenance** | `gpd/results/T-19-attachment-entry-topology.json`, produced by `structure.EntryTopologyStudyKt`; models in `src/main/kotlin/structure/OrigamiMembrane.kt` (added: `EntryBond`, `EntryTopology`, `tetherLoads`, `compatibleShares`) and `origami-engine/src/main/kotlin/structure/JointAllowable.kt`; **25 gate-named tests** in `src/test/kotlin/structure/EntryTopologyTest.kt` (16) and `JointAllowableTest.kt` (9) |
