...
| **Provenance** | `gpd/results/T-148-staple-dropout.json`, produced by `coupling.StapleDropoutStudyKt`; model in `origami-engine/src/main/kotlin/coupling/StapleDropout.kt`, with one additive method (`InfluenceSurrogate.solveWithDropout`) in `origami-engine/src/main/kotlin/coupling/NonUniformCoupling.kt`; the literature transcription, its three validation checks and its URLs in [`gpd/data/T-148-strauss-incorporation-map.md`](../data/T-148-strauss-incorporation-map.md); **9 cheap bounds, 5 incorporation fields, 20 station-field records, 60 Monte Carlo cells at 10 000 realisations each, 4 convergence axes, 17 upstream reproductions, 6 predicates**; **37 gate-named tests in `origami-engine/src/test/kotlin/coupling/StapleDropoutTest.kt`** — **NOTE, the library extraction ([`LIBRARY.md`](../../LIBRARY.md)): one of these, `gate 5 - the 3 x 15 grid's rim mask is C-0058's 34 and 11`, now sits in `src/test/kotlin/coupling/StapleDropoutCorpusCrossCheckTest.kt` because it cross-checks a corpus claim and the library module cannot see the corpus. The test, its name and its assertions are unchanged, and the count is the count of the two files together**; `tools/verify.sh` **BUILD SUCCESSFUL in 17 m 6 s** — the whole suite on its own isolated tree, with a concurrent agent's mid-TDD `src/main/kotlin/stability/RecommendedElementFoldStudy.kt` dropped by `--drop-file`; the result file **re-run through `tools/study.sh` twice and diffed**: three independent runs agree on every one of 2 037 lines and differed only on a `runtimeSeconds` wall clock, which has been **removed** (`CLAUDE.md`: *emit nothing that counts steps*) — the emitted file therefore carries no non-reproducible field, and the removal touched only the study's own `main`, which compiled and ran clean afterwards |
...
Executed as **37 gate-named tests** in `origami-engine/src/test/kotlin/coupling/StapleDropoutTest.kt`.
