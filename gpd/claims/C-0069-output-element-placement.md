...
- **The stability floors are `C-0017`'s, CITED and not recomputed**, and they inherit `C-0005`'s 123–214 % one-loop correction — which is larger than the 6-of-6 against 4-of-6 that separates `Q5` from `Q7`. That count is a **re-reading**, never an establishment of stability at 2 mM. (**RESTATED, `CH-0167`/`C-0137`**: this is an error bar on a LEVEL, and a margin read at a force-pinned operating point is not a level — the same-kind thresholds are a force **1.48–2.22×** smaller or a decay length **9.73 %** shorter.)
