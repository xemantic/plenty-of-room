...
| **Re-read by** | [`C-0049`](C-0049-compliance-ceiling-stroke.md) (`T-107`), which **answers `CH-0047`**: a requirement is owed over `[0, s*]`, the strokes the device traverses, so a device placed at 3 nm never occupies the 4.556 nm stroke `Q2`'s minimum is taken at. Read at the placement stroke this claim's `L3` is **25.227 pN/nm** and clears **4 of the 6** floors at 10 nm / 2 mM, against **0 of 6** for 22.875. **`Q1`, `Q3`, the fold strokes, the 1.0000–1.0019 bias margin and the 0.5 mM recommendation are untouched, and every number reproduces** — what changes is that `Q2`'s failure is model-dependent rather than universal |
...
2. **A beyond-mean-field treatment moving `|k_eff|` at 10 nm.** `C-0005` puts the correction at 123–214 % and gives no direction for an oppositely charged divalent gap. It would not change the *comparison* between the four lines, which is read on one field. (**RESTATED, `CH-0167`/`C-0137`**: this is an error bar on a LEVEL, and a margin read at a force-pinned operating point is not a level — the same-kind thresholds are a force **1.48–2.22×** smaller or a decay length **9.73 %** shorter.)
