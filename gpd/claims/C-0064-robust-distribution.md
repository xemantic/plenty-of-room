# C-0064 — No distribution is flat at all five of `C-0022`'s solved states, and the obstruction is a **sign**: a real minimax reaches **0.1247** of the stroke against `C-0058`'s 0.1587 and stops, because the 2 nm state is the only one of `C-0022`'s 21 whose edge effect is a net **loss**, so its free-tile dishing field is **anti-parallel** to every 10 nm state's. But the five states are **four devices**, not an operating range — and over the range each device actually traverses a robust distribution **exists**, at 0.0372–0.0619 of the stroke, every one inside `T-5b`'s 0.10
...
| **Verification type** | **in-silico** (`C-0009`'s beam-and-hinge grillage and `C-0006`'s continuum plate under `C-0022`'s **solved** load read from its own result file and keyed on concentration, gap **and bias**, driven by an exact **multi-state** Woodbury surrogate with **analytic gradients**, asserted against `C-0058`'s own `InfluenceSurrogate` to `1.2e−15` and against the assembled solve to `8.7e−11`) **+ logical** (a **per-state** least-squares bound in the space of attachment forces, which bounds every distribution whatever, and the per-path force ceiling as arithmetic) |
| **Verdict** | **PASS on the predicate, and the answer is NO at the five states and YES over every operating range.** A smoothed minimax with analytic gradients and **42 starts** — against `C-0058`'s cyclic coordinate descent from three — reaches a worst case of **0.1247** of the free-tile stroke over `C-0022`'s five headline states, **21.4 % better** than `C-0058`'s 0.1587 and still **1.25×** `T-5b`'s 0.10. **The search is not what limits it**: 16 starts to 42 moves the answer by `1.1e−9`, the six-level smoothing homotopy is worth 6.4 %, and **two states are active at the optimum**, which is what an equalised minimax looks like. **The obstruction is a sign.** Of the 31 non-empty subsets, **every one of the 14 that puts the 2 nm state together with a 10 nm state fails** (0.1085–0.1252) and **every one of the other 17 is flat** (0.0090–0.0797) — including everything-but-the-2-nm-state at **0.0797** and the 2 nm state paired with the 5 nm rest state, *its own device*, at **0.0619**. The 2 nm state's free-tile dishing field has a cosine of **−0.943 to −1.000** against every other state — exactly **−1.000** against the 10 mM one — while all six pairs among the other four run **+0.949 to +0.997**; and `C-0022`'s own table says why: the 2 nm state is the **only one of its 21** whose finite tile carries **less** total force than a 1-D pressure over the footprint (−3.91 %). **But the five states are not an operating range.** Three are the *rest* states of three different buffers at a 10 nm layer and two are the *rest and held* states of a **5 nm** layer; the 2 nm state is that device at §3's 3 nm stroke and no state of the 10 nm device at all. Over the range each device traverses — one buffer, one layer, one bias, `L₀` down to `L₀ − s` — the minimax is **0.0372** (`C-0018`'s placed 2 mM device), **0.0436** (`C-0032`'s 0.5 mM recommendation), **0.0619** (the 5 nm device) and **0.0500** (10 mM): **all four inside `T-5b`'s 0.10**, and adding two interpolated intermediate gaps moves the first by `2.4e−15`. **A real failure is reported on buildability**: the 45-parameter robust optimum spans **7.95×** and does **not** survive `C-0060`'s two levels — quantised optimally onto two it measures **0.1002**, outside the convention by 0.2 %, at a level ratio of **2.475**, below `C-0060`'s measured `3.5 ≤ R ≤ 20` window — and needs **three** levels (0.0741, ratio 3.633, inside the window) to be flat. **What rescues it is that the buildable family must be searched in, not projected onto**: `C-0058`'s own two-level rim × 5 rule is flat over the whole traversed range at **0.0753**, 24.8 % better than the projected optimum and already priced buildable by `C-0060`. |
...
| **Provenance** | `gpd/results/T-123-robust-distribution.json`, produced by `coupling.RobustDistributionStudyKt`; model in `src/main/kotlin/coupling/RobustDistribution.kt`; **5 state records, 10 cheap-bound records, 20 tension records, 31 subset records (every non-empty subset of the five), 4 operating-range records, 8 assembled solves, 90 path records, 8 buildability records, 18 convergence records, 18 upstream reproductions**; **15 gate-named tests in `src/test/kotlin/coupling/RobustDistributionTest.kt`**; `tools/verify.sh` **BUILD SUCCESSFUL** on its own isolated tree — the whole suite, on the finished tree — with a concurrent agent's mid-TDD test dropped by `--drop-file` (`src/test/kotlin/anchoring/CrossbarTrioExistenceTest.kt`, `T-127`); the result file re-run through `tools/study.sh` and diffed **byte-for-byte identical on two independent runs** |
...
| | `C-0058` | **here** | change |
|---|---|---|---|
| **minimax over the five solved states**, / stroke | 0.1587 | **0.1247** | **−21.4 %**, still **1.25×** the tolerance |
| single-state optimum at the design point, / stroke | 0.0544 | **0.0214** | **2.54× better**, at the same state and the same 45 paths |

The five per-state optima are **0.0214 / 0.0219 / 0.0281 / 0.0144 / 0.0090** — *every state is easy on
its own*, by a factor of 4 to 11 inside the convention. The difficulty is entirely in sharing.
...
|---|---|---|---|
| starts | 1 / 6 / 16 / **42** | 0.12479 / 0.12489 / 0.12471 / **0.12471** | `1.1e−9` from 16 to 42 |
| smoothing homotopy | 1 level / 2 levels / **6 levels** | 0.1327 / 0.1284 / **0.1247** | worth **6.4 %** |
| sampling grid | 41 / 81 / **161** | 0.12471 / 0.12471 / 0.12473 | `1.9e−4` |
| **NESTED** subdivisions | 1 ⊂ 2 ⊂ **4** | 0.12521 / 0.12471 / 0.12473 | `1.2e−4` |
...
|---|---|---|---|
| **contains the 2 nm state AND a 10 nm state** | **14** | **0.1085 – 0.1252** | **none** |
| everything else | **17** | 0.0090 – **0.0797** | **all** |

The dichotomy is exact, and it **recovers the devices**:

- the three 10 nm rest states together: **0.0435**, flat;
- all four states other than the 2 nm one: **0.0797**, flat;
- the 2 nm state paired with the 5 nm rest state — **its own device** — : **0.0619**, flat;
- the 2 nm state paired with *any* 10 nm state: 0.1085–0.1231, **never** flat.
...
| pair | cosine |
|---|---|
| 2 nm ↔ 10 mM, 10 nm | **−1.000** |
| 2 nm ↔ 2 mM, 10 nm | −0.984 |
| 2 nm ↔ 0.5 mM, 10 nm | −0.966 |
| 2 nm ↔ 2 mM, 5 nm | −0.943 |
| every pair among the other four | **+0.949 to +0.997** |
...
|---|---|---|---|---|---|
| **2 mM, `L₀` = 10 nm, 0.192 V** (`C-0018`'s placed device) | 10 nm, 7 nm | **0.0372** | **YES** | 0.2182 | 0.0753 |
| **0.5 mM, `L₀` = 10 nm, 0.134 V** (`C-0032`'s recommendation) | 10 nm, 7 nm × 2 biases | **0.0436** | **YES** | 0.2086 | 0.0683 |
| **2 mM, `L₀` = 5 nm, 0.368 V** | 5 nm, 2 nm | **0.0619** | **YES** | 0.0796 | 0.1867 |
| **10 mM, `L₀` = 10 nm, 0.192 V** | 10 nm, 7 nm × 2 biases | **0.0500** | **YES** | 0.2551 | 0.1179 |

Three things follow.

1. **A robust distribution exists for every device in `C-0022`'s box**, at 0.0372–0.0619 — 1.6–2.7×
...
3. **The range's discretisation is not what decides it.** Adding two linearly interpolated intermediate
   gaps to the 2 mM device's range moves its optimum's worst case by **`2.4e−15`** — the collar family
   is smooth in the gap, so two solved endpoints are the range.
...
| design | levels | stiffness values [pN/nm] | ratio | inside `C-0060`'s window? | worst / stroke | flat? |
|---|---|---|---|---|---|---|
| robust optimum, quantised | **2** | 0.4146, 1.0261 | **2.475** | **no** | **0.1002** | **NO**, by 0.2 % |
| robust optimum, quantised | **3** | 0.2961, 0.6360, 1.0756 | 3.633 | **yes** | **0.0741** | **YES** |
| robust optimum, quantised | 4 | 0.2961, 0.5890, 0.9206, 1.1923 | 4.027 | yes | 0.0682 | YES |
| the robust optimum itself | 45 | 0.171 … 1.358 | 7.946 | — | **0.0372** | YES |
| **`C-0058`'s rim × 5 over 6.70 nm** | **2** | **0.1842, 0.9208** | **5.000** | **yes** | **0.0753** | **YES** |

**This is a real failure and it is reported as one: the 45-parameter robust optimum does not survive
`C-0060`'s two levels.** Quantised optimally onto two it misses the convention (0.1002 against 0.10)
and its level ratio falls *below* `C-0060`'s measured window; it needs a genuine **third** level to be
flat, where `C-0060` prices two. `C-0060`'s own 3–4 distinct settings are ±1 bp *trims* of the same two
levels, not a third level, so this is a new demand in kind.

**What answers it is that a constrained family must be searched in, not projected onto.**
`C-0058`'s own published two-level rule — the 34 stations within 6.7 nm of an edge carrying 5× the
other 11, ratio exactly 5, the design `C-0060` shows every catalogue element can build — is flat over
the **whole traversed range** of both 10 nm devices, at **0.0753** and **0.0683**. That is **24.8 %
better than the projected optimum** and needs nothing new. The buildability failure therefore does not
reopen the range verdict; it says that the flat *buildable* robust design is one that already exists.

> `C-0058`'s rim rule is **not** flat over the 5 nm device's range (0.1867) or the 10 mM one (0.1179).
> Those two devices need a distribution of their own, and this study supplies one for each.

### The load paths, at the robust optimum

| | uniform | **robust optimum** | allowable |
|---|---|---|---|
| peak path stiffness [pN/nm] | 0.741 | **1.358** | 3.333 (`= a/s`) |
| **peak path force at the 3 nm stroke** [pN] | 2.222 | **4.073** | **10** — margin **2.45×** |
| peak crossover force [pN] | 0.150 | **0.781** | 10 — margin **12.8×** |
| peak duplex shear [pN] | 0.793 | **1.088** | 48–65 |
| **peak per-path thermal force** [pN] | 0.261 | **0.479** (**+83 %**) | — |

- `C-0049`'s admissible ratio `n·a/(s·K)` is **4.50** at §3's acceptable 3 nm and **1.35** at its
  desired 10 nm, tightening as `1/s` exactly. The robust optimum's stiffest path is **1.833×** the
...
- `C-0014`'s over-stiffening penalty is **linear** in a path's share, so the **+83 %** thermal premium
...
Executed as **15 gate-named tests** in `src/test/kotlin/coupling/RobustDistributionTest.kt`.
...
| 4 | **the real optimiser failing to beat `C-0058`'s 0.1587** | **no** | 0.1247, and it is asserted in a test |
...
3. The Plan expected the robust distribution to be buildable on `C-0060`'s two levels. It is not, and
   the escape is that `C-0058`'s existing two-level design is *already* flat over the range — i.e.
   **projection onto a constrained family is not optimisation within it**, and here the gap is 24.8 %.
...
- **`T-5b`'s 10 % is a CONVENTION.** At 5 % the 2 mM and 0.5 mM device ranges survive (0.0372, 0.0436)
  and the 5 nm one does not (0.0619); `C-0058`'s buildable rim rule survives at 8 % and not at 5 %.
...
4. **A two-level design searched over the range** rather than at the design point. `C-0058`'s ratio 5 is
   flat over the range at 0.0753 and `C-0060` found ratio **7** better at the design point (0.0653); the
   best two-level member *of the range problem* is not known.
