...
It lives in `gpd/data/` and not in `tools/` deliberately: `tools/P-31-harness-census.py` refuses
an undeclared `tools/*mutation-test.py`, and `CLAUDE.md` records that a harness written by an
agent who may not touch that registry belongs here. Moving it is one tuple in `HARNESSES` and it
is a queue item (`T-309`), never a deletion.

usage: gpd/data/T-303-mutation/mutate.py <snapshot-dir>
...
def main():
    if len(sys.argv) != 2:
        print("usage: gpd/data/T-303-mutation/mutate.py <snapshot-dir>", file=sys.stderr)
        sys.exit(__doc__)
    tree = sys.argv[1]
