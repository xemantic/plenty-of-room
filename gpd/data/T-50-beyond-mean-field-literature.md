...
**`T-221` SETTLED IT (iteration 34, [`C-0143`](../claims/C-0143-planar-coupling-wall.md)): the criterion is owed at the BARE charge, and the `16.5×` is 100 % the bare/renormalised axis and 0 % the cylinder/plane axis this paragraph frames it as.** `Ξ` and `D̃` are both linear in `σ`, so `Ξ/D̃ = q²l_B/D` carries no wall convention and Eq. (64) is exactly `ln(D/μ) < D/(q²l_B)`; swept as a `2 × 2`, all four bare readings fail at 5, 7 and 10 nm and both renormalised ones pass. The paper's own Eq. (14) fixes the counterion count by electroneutrality against `σ₁ + σ₂`, so a renormalised `σ` deletes exactly the population `Ξ` counts. Row 25's own **primary** is retained in [`T-50-sources/`](T-50-sources/MANIFEST.md) so the derivations `T-221` takes from Eqs. (14) and (18) are checkable.
